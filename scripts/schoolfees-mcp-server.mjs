...
async function buildDailyRecoveryDigest({
  ...
  const [collectionSummary, recoveryQueue, promisesDue, recoveryPlan, followUpDrafts] =
    await Promise.all([
      buildCollectionBrief({
        sessionLabel,
        topDefaultersLimit: 10,
        recentPaymentsLimit: 10,
      }),
      buildRecoveryQueue({ sessionLabel, limit: recoveryLimit, includeNoCall: false }),
      buildPromiseDueList({ sessionLabel, limit: promiseLimit }),
      buildRecoveryPlan({ sessionLabel, limit: recoveryLimit, language }),
      buildFollowupDrafts({
        sessionLabel,
        minPendingAmount: 0,
        overdueOnly: false,
...
function createServer() {
  ...
  server.registerTool(
    ...
    async ({ sessionLabel, topDefaultersLimit, recentPaymentsLimit }) => {
      const brief = await buildCollectionBrief({
        sessionLabel,
        topDefaultersLimit,
        recentPaymentsLimit,
      });
  ...
  server.registerTool(
    ...
    async ({ sessionLabel, limit, includeNoCall }) => {
      const queue = await buildRecoveryQueue({ sessionLabel, limit, includeNoCall });
  ...
  server.registerTool(
    ...
    async ({ sessionLabel, limit }) => {
      const promiseList = await buildPromiseDueList({ sessionLabel, limit });
  ...
  server.registerTool(
    ...
    async ({ sessionLabel, limit, language }) => {
      const plan = await buildRecoveryPlan({ sessionLabel, limit, language });
  ...
  server.registerTool(
    ...
    async ({ sessionLabel, minPendingAmount, overdueOnly, limit, language }) => {
      const draftPack = await buildFollowupDrafts({
        sessionLabel,
        minPendingAmount,
        overdueOnly,
        limit,
  ...
  server.registerTool(
    "daily_recovery_digest",
    {
      ...
      inputSchema: {
        sessionLabel: SessionLabel,
        language: z.enum(["english", "hinglish"]).default("hinglish"),
        recoveryLimit: Limit.default(25),
        promiseLimit: Limit.default(25),
        draftLimit: Limit.default(10),
      },
      annotations: readOnly,
    },
    async ({ sessionLabel, language, recoveryLimit, promiseLimit, draftLimit }) => {
      const digest = await buildDailyRecoveryDigest({
        sessionLabel,
        language,
        recoveryLimit,
        promiseLimit,
