...
After running it, re-check by visiting Dashboard or by re-running the verify query in `docs/imports/2026-05-15-RUN-THIS-TO-FINISH.md`. Expected outcome: `missing_installments` for active 2026-27 → 0.
...
| 4 | **37 Left_Students rows had no matching `public.students`** | `docs/imports/2026-05-15-latest-vpps-import/apply-payloads/_left-resolution.json` | Workbook-side check; harmless to ignore (they were never in DB) |
| 5 | **9 in-workbook student duplicates** | `docs/import-previews/2026-05-15-latest-vpps-import/anomalies.csv` | Workbook fix for next run |
