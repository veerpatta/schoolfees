...
**EDIT LIST:** `package.json` (scripts only), `scripts/school-one/dev-db.mjs` (new), `scripts/school-one/dev-unschedule-external-jobs.sql` (new), `supabase/seeds/04_school_one_dev_seed.sql` (new), `docs/school-one/dev-environment.md` (new), `.env.example` (add names only).
...
2. `dev-db.mjs`: reads `SUPABASE_DEV_PROJECT_REF` from env; **refuses to run** if the ref equals `PRODUCTION_SUPABASE_PROJECT_REF` or `vgqyilgstjvgohrsiwkb` (hard-coded second check on purpose); runs `npx supabase link --project-ref <dev>` then `npx supabase db push --yes` for `push`; `npx supabase db reset --linked` for `reset` (requires typing the dev ref back as confirmation); `seed` applies `supabase/seeds/*.sql` in order via `psql`/`supabase db query` against the dev project, then runs `dev-unschedule-external-jobs.sql`.
3. `dev-unschedule-external-jobs.sql`: `select cron.unschedule(jobname) from cron.job where jobname in ('notion-fee-sync-daily','notion-fee-sync-daily-test');` — dev must never call Notion. Leave the other pg_cron jobs scheduled (they are harmless and useful in dev).
4. `04_school_one_dev_seed.sql`: extend the existing test seeds (read `01_test_session_setup.sql`, `02_test_students_seed.sql` first and reuse their session and class rows) so dev has: an `admin` user via the existing bootstrap mechanism, ~60 fake students across 6 classes with obviously fake names (`Student A01` … ), fake phone numbers in the `+91 00000 000NN` pattern, no Aadhaar values. Keep it idempotent.
...
**Acceptance:** `npm run db:push:dev` against the dev project applies all migrations with zero errors and `supabase migration list` shows local = remote; `npm run db:seed:dev` is idempotent (second run makes no changes); `dev-db.mjs` with the production ref exits non-zero before doing anything; existing tests green.
...
4. Wrap the two existing cron routes: same behaviour and same response bodies as today (diff the responses in tests), just inside `runJob()` and using `requireJobSecret()` with `allowQuery: true`. `maxDuration` unchanged.
...
**Acceptance:** migration applies on the local stack and dev; `schema:snapshot --check` passes; both cron routes behave identically for a caller using `CRON_SECRET` and for a caller using the new per-job secret; `job_runs` shows a row per invocation; tests green.
...
5. **Vercel Production:** add `JOB_SECRET_BACKUP_REPORT` (same value as GitHub), `JOB_SECRET_NIGHTLY_BACKUP`, `JOB_SECRET_AUTO_DAY_CLOSE`.
...
**Task:**
1. Workflow: `schedule: '0 21 1 * *'` (02:30 IST on the 1st) + `workflow_dispatch`; `environment: backup`; service container `supabase/postgres` at the tag matching production's Postgres version (production reports `17.6.1.121`; verify the tag exists on Docker Hub and fall back to the nearest `17.x` tag, noting the choice). Fallback if the image cannot start in CI: `postgres:17` plus a pre-script that creates the roles `anon`, `authenticated`, `service_role`, `supabase_admin`, schemas `auth`, `extensions`, `private`, and stub functions `auth.uid()`, `auth.role()`, `auth.jwt()` so policies load.
