...
| Run id | `202609051145` |
| Layers | static |
| Git | `0533d42bb629` on main (dirty) |
| Node | v26.7.0 |
| Files read | 1614 |
| Started | 2026-09-05T11:45:37.555Z |
| Duration | 14299 ms |
| Strict gate | no |
...
10 check(s) swept 14869 of 14869 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 120 | 120 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 757 | 757 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 757 | 757 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 757 | 757 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 724 | 724 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 231 | 231 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1416 | 1416 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7713 | 7713 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2383 | 2383 | exhaustive |
