...
| Run id | `202608220504` |
| Layers | static |
| Git | `a6ff249b44fd` on whatsapp-v2-reminders-section (dirty) |
| Node | v24.19.0 |
| Files read | 1530 |
| Started | 2026-08-22T05:04:42.947Z |
| Duration | 5559 ms |
| Strict gate | no |
...
10 check(s) swept 14504 of 14504 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 115 | 115 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 726 | 726 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 726 | 726 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 726 | 726 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 694 | 694 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 216 | 216 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1356 | 1356 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7708 | 7708 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2226 | 2226 | exhaustive |
...
### P2-025 src/modules/whatsapp/data/campaign-store.ts:120 exports getCampaign, which nothing uses

```
id:         60f66d16cc4a
rule:       scan.dead-export  [deterministic]  layer: static
surface:    src/modules/whatsapp/data/campaign-store.ts:120
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `getCampaign` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside src/modules/whatsapp/data/campaign-store.ts, at its own declaration. 6 module(s) import this file, none of them for this name.
source:     export async function getCampaign(supabase: any, id: string): Promise<SavedCampaign | null> {
