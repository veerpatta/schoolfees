# Source scan 202609130626

**PASS** · 55 finding(s) · P0 0 · P1 0 · P2 44 · P3 11
...
| Run id | `202609130626` |
| Layers | static |
| Git | `d5dabbee70df` on main (dirty) |
| Node | v24.19.0 |
| Files read | 1625 |
| Started | 2026-09-13T06:26:01.237Z |
| Duration | 19073 ms |
| Strict gate | no |
...
11 check(s) swept 15032 of 15032 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 124 | 124 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 775 | 775 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 775 | 775 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 775 | 775 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 743 | 743 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 236 | 236 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1454 | 1454 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7601 | 7601 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2530 | 2530 | exhaustive |
...
### P2-003 src/app/api/cron/whatsapp-scheduled-runs/route.ts is a cron handler that nothing schedules
...
### P2-004 src/app/protected/dashboard/page.tsx:548 formats a rupee figure without lib/helpers/currency.ts
...
### P2-005 src/app/protected/dashboard/page.tsx:550 formats a rupee figure without lib/helpers/currency.ts
...
### P2-006 src/app/protected/dashboard/page.tsx:551 formats a rupee figure without lib/helpers/currency.ts
...
### P2-007 src/app/protected/defaulters/actions.ts:255 exports logWhatsAppSendAttempts, which nothing uses

```
id:         cea516efbc74
rule:       scan.dead-export  [deterministic]  layer: static
surface:    src/app/protected/defaulters/actions.ts:255
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `logWhatsAppSendAttempts` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside src/app/protected/defaulters/actions.ts, at its own declaration. 4 module(s) import this file, none of them for this name.
source:     export async function logWhatsAppSendAttempts(
...
### P2-008 src/app/protected/settings/page.tsx:160 formats a rupee figure without lib/helpers/currency.ts
...
### P2-009 src/app/protected/students/[studentId]/edit/page.tsx:302 formats a rupee figure without lib/helpers/currency.ts
...
### P2-010 src/app/protected/students/repayment-plan-actions.ts:327 formats a rupee figure without lib/helpers/currency.ts
...
### P2-011 src/modules/dashboard/domain/summary.ts:162 formats a rupee figure without lib/helpers/currency.ts

```
id:         81b66fc3ef28
rule:       scan.money-format-raw  [deterministic]  layer: static
surface:    src/modules/dashboard/domain/summary.ts:162
expected:   Every rupee a person reads is produced by formatInr() or <Money />, and every other en-IN grouped number by the plain formatter beside it, so a find-references on src/platform/helpers/currency.ts reaches every one of them.
actual:     This line uses a hand-written "Rs" literal with no period directly. scripts/audit-money-formatting.mjs matches only "Rs." with the period (/["'`]Rs\.\s*\d|>\s*Rs\.\s/), so this spelling passes CI today.
...
### P2-012 src/modules/fees/data/setup-queries.ts:191 formats a rupee figure without lib/helpers/currency.ts
...
### P2-013 src/modules/fees/data/workbook-setup-change.ts:125 formats a rupee figure without lib/helpers/currency.ts
...
### P2-014 src/modules/fees/data/workbook-setup-change.ts:161 formats a rupee figure without lib/helpers/currency.ts
...
### P2-015 src/modules/fees/data/workbook-setup-change.ts:404 formats a rupee figure without lib/helpers/currency.ts
...
### P2-016 src/modules/fees/data/workbook-setup-change.ts:418 formats a rupee figure without lib/helpers/currency.ts
...
### P2-017 src/modules/fees/domain/policy-shaping.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-018 src/modules/imports/domain/parser.ts:133 formats an en-IN number without lib/helpers/currency.ts
...
### P2-019 src/modules/imports/domain/parser.ts:145 formats an en-IN number without lib/helpers/currency.ts
...
### P2-020 src/modules/payments/data/queries.ts:242 formats a rupee figure without lib/helpers/currency.ts

```
id:         86b75da6d52f
rule:       scan.money-format-raw  [deterministic]  layer: static
surface:    src/modules/payments/data/queries.ts:242
expected:   Every rupee a person reads is produced by formatInr() or <Money />, and every other en-IN grouped number by the plain formatter beside it, so a find-references on src/platform/helpers/currency.ts reaches every one of them.
actual:     This line uses toLocaleString("en-IN") directly. scripts/audit-money-formatting.mjs enforces the same rule, but only walks app/ and components/ — it never reads src/.
...
### P2-021 src/modules/payments/domain/bulk/template.ts:138 formats a rupee figure without lib/helpers/currency.ts
...
### P2-022 src/modules/payments/domain/payment-desk-workflow.ts:100 formats a rupee figure without lib/helpers/currency.ts
...
### P2-023 src/modules/promotion/data/queries.ts:834 formats a rupee figure without lib/helpers/currency.ts
...
### P2-024 src/modules/promotion/data/queries.ts:975 formats a rupee figure without lib/helpers/currency.ts
...
### P2-025 src/modules/receipts/domain/share-docs.ts:26 exports buildReceiptShareDocs, which nothing uses

```
id:         36df686c4c7b
rule:       scan.dead-export  [deterministic]  layer: static
surface:    src/modules/receipts/domain/share-docs.ts:26
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `buildReceiptShareDocs` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside src/modules/receipts/domain/share-docs.ts, at its own declaration. Nothing imports this module at all.
source:     export function buildReceiptShareDocs({
why:        Dead code is not neutral here. It compiles, it is type-checked, it is counted against the source-line budgets in quality/office-quality-budgets.json, and the next reader takes it for something that runs — so a half-finished extraction reads as a finished one, and a retired module reads as a live one.
fix:        Delete it, or wire up the caller it was written for. If it is deliberately kept as an entry point for something outside this repo's import graph, say so in a comment beside the export — this rule reads the whole tree and will keep reporting it otherwise.
```

### P2-026 src/modules/repayment-plans/domain/schedule.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-027 src/modules/students/ui/student-form.tsx:609 formats a rupee figure without lib/helpers/currency.ts
...
### P2-028 src/modules/students/ui/student-form.tsx:612 formats a rupee figure without lib/helpers/currency.ts
...
### P2-029 src/modules/whatsapp/data/delivery-store.ts:223 exports loadSeenButNotPaid, which nothing uses
...
### P2-030 src/platform/config/fee-rules.ts:180 formats a rupee figure without lib/helpers/currency.ts
...
### P2-031 src/app/protected/students/close-due-actions.ts:47 rounds `value` where the domain core truncates it
...
### P2-032 src/platform/helpers/amount-in-words-hi.ts:73 rounds `value` where the domain core truncates it
...
### P2-033 src/platform/helpers/amount-in-words.ts:84 rounds `value` where the domain core truncates it
...
### P2-034 src/platform/pdf/document-kit.tsx:55 rounds `value` where the domain core truncates it

```
id:         7fa0faf06ed5
rule:       scan.rounding-policy-mixed  [heuristic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:55
expected:   One rounding policy per rupee. The domain core coerces a money value to whole rupees with Math.trunc — lib/fees/due-amounts.ts, lib/receipts/amounts.ts, src/lib/finance/financial-state.ts and lib/payments/allocation.ts all do — so a figure reaches the ledger, the receipt and the export with the same value.
actual:     This coerces `value` with Math.round, while 10 other sites coerce the same-named quantity with Math.trunc (src/modules/fees/data/conventional-discounts.ts:167, src/modules/fees/domain/conventional-discount-rules.ts:16, src/modules/fees/domain/due-amounts.ts:36). For 1500.6 one answers 1501 and the other 1500.
source:     return Math.round(value || 0).toLocaleString("en-IN"); // @allow-raw-money-format: this IS the PDF money formatter; react-pdf renders no DOM, so formatInr() cannot be used here.
why:        Latent today and not later: every money column in supabase/migrations is `integer`, so both policies agree on everything currently in the database. The first fractional value that reaches one of these — an imported spreadsheet, a percentage discount, a future decimal column — makes the receipt and the ledger differ by a rupee, with nothing in either to say which is right.
fix:        Use Math.trunc here too, or state in a comment why this surface deliberately rounds up and the ledger does not.
```

### P2-035 private.enforce_max_active_conventional_discounts is defined without a pinned search_path
...
### P2-036 private.prevent_receipt_adjustment_mutation is defined without a pinned search_path
...
### P2-037 private.enforce_max_active_conventional_discounts_in_schema is defined without a pinned search_path
...
### P2-038 private.enforce_third_child_traceability is defined without a pinned search_path
...
### P2-039 private.derive_family_child_client_request_id is defined without a pinned search_path
...
### P2-040 public.refresh_financial_materialized_views is defined without a pinned search_path
...
### P2-041 private.prevent_notion_sync_log_mutation is defined without a pinned search_path
...
### P2-042 public.v_notion_student_fee_summary does not set security_invoker
...
### P2-043 public.v_notion_daily_collection_summary does not set security_invoker
...
### P2-044 public.v_notion_family_fee_summary does not set security_invoker
...
### P3-045 src/app/protected/admin-tools/session-health/session-health-grid.tsx hardcodes the live session 2026-27
...
### P3-046 src/app/protected/exports/page.tsx declares revalidate = 60, which never takes effect
...
### P3-047 src/app/protected/fee-setup/page.tsx declares revalidate = 60, which never takes effect
...
id:         4cfbe989fcfe
rule:       scan.observation  [heuristic]  layer: static
surface:    src/app/protected/students/[studentId]/page.tsx:1414
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
...
### P3-049 src/app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27

```
id:         d225fce88a2f
rule:       scan.observation  [heuristic]  layer: static
surface:    src/app/protected/students/[studentId]/page.tsx:1556
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
actual:     "2026-27" appears as a fallback for a value read at runtime. Nothing in this module writes through a Supabase client and it is not a "use server" action, so no ledger is at risk — this is inventory of where the live year is baked in, not a defect.
source:     sessionLabel={financialSnapshot?.policy.academicSessionLabel || "2026-27"}
why:        Every hardcoded copy is a place the AY 2027-28 rollover has to find. It also quietly decides for a reader which session they are looking at: a fallback or an is-current comparison that names 2026-27 keeps saying so after the school has moved on.
...
### P3-051 src/platform/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-052 workers/schoolfees-mcp/src/prompts.mjs hardcodes the live session 2026-27
...
### P3-053 workers/schoolfees-mcp/src/toolkit.mjs hardcodes the live session 2026-27
...
### P3-054 workers/schoolfees-mcp/src/tools/orientation.mjs hardcodes the live session 2026-27
...
### P3-055 workers/schoolfees-mcp/worker.mjs hardcodes the live session 2026-27
...
| Rule | Severity | Count |
|---|---|---:|
| `scan.money-format-raw` | P2 | 24 / 25 |
| `scan.sql-risk` | P2 | 10 / 10 |
| `scan.rounding-policy-mixed` | P2 | 4 / 4 |
| `scan.config-risk` | P2 | 3 / 3 |
| `scan.dead-export` | P2 | 3 |
