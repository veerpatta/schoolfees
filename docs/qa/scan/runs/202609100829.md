...
| Run id | `202609100829` |
| Layers | static |
| Git | `bd7e0c756e84` on main (dirty) |
| Node | v24.19.0 |
| Files read | 1625 |
| Started | 2026-09-10T08:29:23.208Z |
| Duration | 6470 ms |
| Strict gate | no |
...
10 check(s) swept 15051 of 15051 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 122 | 122 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 773 | 773 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 773 | 773 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 773 | 773 | exhaustive |
...
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1438 | 1438 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7713 | 7713 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2479 | 2479 | exhaustive |
