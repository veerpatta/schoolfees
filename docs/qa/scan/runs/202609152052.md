# Source scan 202609152052

**FAIL** · 56 finding(s) · P0 0 · P1 0 · P2 45 · P3 11
...
| Run id | `202609152052` |
| Layers | static |
| Git | `6f79162ce675` on school-one/phase-0 (dirty) |
| Node | v24.19.0 |
| Files read | 1656 |
| Started | 2026-09-15T20:52:00.098Z |
| Duration | 14983 ms |
| Strict gate | no |

## Verdict

The gate **failed** for 1 reason(s):

- scan.sql-risk regressed: 12 findings against a baseline budget of 10.

## What this scan did NOT look at

10 check(s) swept 15091 of 15091 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 125 | 125 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 780 | 780 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 780 | 780 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 780 | 780 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 748 | 748 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 239 | 239 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1468 | 1468 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7613 | 7613 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2547 | 2547 | exhaustive |
...
### P2-033 src/platform/pdf/document-kit.tsx:55 rounds `value` where the domain core truncates it
...
### P2-041 public.v_notion_daily_summary does not set security_invoker

```
id:         4dfccb928c9a
rule:       scan.sql-risk  [heuristic]  layer: static
surface:    supabase/migrations/20260612023100_notion_sync_views_that_never_had_a_migration.sql:34
...
actual:     The live definition of public.v_notion_daily_summary — supabase/migrations/20260612023100_notion_sync_views_that_never_had_a_migration.sql:34 — sets no `security_invoker` option, so it runs with its owner's privileges.
source:     create or replace view public.v_notion_daily_summary as
...
### P2-042 public.v_notion_student_fee_sync does not set security_invoker

```
id:         9d20ea01ef47
rule:       scan.sql-risk  [heuristic]  layer: static
surface:    supabase/migrations/20260612023100_notion_sync_views_that_never_had_a_migration.sql:50
expected:   A plain view over student or financial tables declares `with (security_invoker = true)`, as 25 of the 29 live plain views here do.
actual:     The live definition of public.v_notion_student_fee_sync — supabase/migrations/20260612023100_notion_sync_views_that_never_had_a_migration.sql:50 — sets no `security_invoker` option, so it runs with its owner's privileges.
source:     create or replace view public.v_notion_student_fee_sync as
why:        Without security_invoker a view reads its base tables as the view owner, and RLS on students, installments, payments and receipts is simply not consulted. Anyone who can select from the view sees every row it can reach, whatever their own policies say.
fix:        Recreate the view with `with (security_invoker = true)` in a new migration — or, if it is deliberately a controlled escape hatch for a service role, say so in a comment on the view so the next reader sees a decision instead of an omission.
```

### P2-043 public.v_notion_student_fee_summary does not set security_invoker
...
### P2-044 public.v_notion_daily_collection_summary does not set security_invoker
...
### P2-045 public.v_notion_family_fee_summary does not set security_invoker
...
### P3-046 src/app/protected/admin-tools/session-health/session-health-grid.tsx hardcodes the live session 2026-27
...
### P3-047 src/app/protected/exports/page.tsx declares revalidate = 60, which never takes effect
...
### P3-048 src/app/protected/fee-setup/page.tsx declares revalidate = 60, which never takes effect
...
### P3-050 src/app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27
...
### P3-052 src/platform/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-053 workers/schoolfees-mcp/src/prompts.mjs hardcodes the live session 2026-27
...
### P3-054 workers/schoolfees-mcp/src/toolkit.mjs hardcodes the live session 2026-27
...
### P3-055 workers/schoolfees-mcp/src/tools/orientation.mjs hardcodes the live session 2026-27
...
### P3-056 workers/schoolfees-mcp/worker.mjs hardcodes the live session 2026-27
