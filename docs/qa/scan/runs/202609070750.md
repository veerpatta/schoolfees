...
| Run id | `202609070750` |
| Layers | static |
| Git | `6e1b2ece7f6c` on main (dirty) |
| Node | v24.19.0 |
| Files read | 1607 |
| Started | 2026-09-07T07:50:18.802Z |
| Duration | 8355 ms |
| Strict gate | no |
...
10 check(s) swept 14940 of 14940 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 122 | 122 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 764 | 764 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 764 | 764 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 764 | 764 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 732 | 732 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 231 | 231 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1426 | 1426 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7713 | 7713 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2413 | 2413 | exhaustive |
...
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `istTodayIso` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside src/platform/helpers/date.ts, at its own declaration. 71 module(s) import this file, none of them for this name.
