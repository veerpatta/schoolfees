...
| Run id | `202608220047` |
| Layers | static |
| Git | `cf140924b8ff` on whatsapp-six-notices (dirty) |
| Node | v24.19.0 |
| Files read | 1513 |
| Started | 2026-08-22T00:47:32.598Z |
| Duration | 5429 ms |
| Strict gate | no |
...
10 check(s) swept 14401 of 14401 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 111 | 111 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 717 | 717 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 717 | 717 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 717 | 717 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 683 | 683 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 214 | 214 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1342 | 1342 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7699 | 7699 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2190 | 2190 | exhaustive |
