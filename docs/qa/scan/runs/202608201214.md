...
| Run id | `202608201214` |
| Layers | static |
| Git | `4028da4f45ba` on main (dirty) |
| Node | v24.19.0 |
| Files read | 1494 |
| Started | 2026-08-20T12:14:54.508Z |
| Duration | 9101 ms |
| Strict gate | no |
...
11 check(s) swept 14417 of 14417 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 111 | 111 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 720 | 720 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 720 | 720 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 720 | 720 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 688 | 688 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 211 | 211 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1342 | 1342 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7710 | 7710 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2181 | 2181 | exhaustive |
