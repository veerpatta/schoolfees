# Source scan 202608191413

**FAIL** · 132 finding(s) · P0 0 · P1 12 · P2 108 · P3 12
...
| Node | v24.19.0 |
| Files read | 1471 |
| Started | 2026-08-19T14:13:44.668Z |
| Duration | 9708 ms |
| Strict gate | no |

## Verdict

The gate **failed** for 13 reason(s):

- P1 scan.error-swallowed on app/protected/admin-tools/promotion/actions.ts:60 (static, deterministic).
- P1 scan.error-swallowed on app/protected/fee-setup/actions.ts:351 (static, deterministic).
- P1 scan.error-swallowed on app/protected/fee-setup/actions.ts:352 (static, deterministic).
- P1 scan.error-swallowed on app/protected/master-data/actions.ts:156 (static, deterministic).
- P1 scan.money-round-then-validate on app/protected/payments/waive-late-fee-actions.ts:34 (static, deterministic).
- P1 scan.money-split-not-conserving on app/protected/students/[studentId]/page.tsx:509 (static, deterministic).
- P1 scan.money-round-then-validate on app/protected/students/close-due-actions.ts:25 (static, deterministic).
- P1 scan.money-split-not-conserving on components/payments/mobile-payment-flow-sheet.tsx:1112 (static, deterministic).
- P1 scan.dependency-vulnerable on package.json:114 (static, deterministic).
- P1 scan.dependency-vulnerable on package.json:117 (static, deterministic).
- P1 scan.dependency-vulnerable on package.json:57 (static, deterministic).
- P1 scan.dependency-vulnerable on package.json:57 (static, deterministic).
- scan.config-risk regressed: 3 findings against a baseline budget of 2.

## What this scan did NOT look at

11 check(s) swept 14309 of 14309 enumerated members.
...
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 209 | 209 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1322 | 1322 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7703 | 7703 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2140 | 2140 | exhaustive |
| `config-risk` | deployment and build configuration candidates | 11 | 11 | exhaustive |
| `deps` | npm advisories across the installed dependency tree | 11 | 11 | exhaustive |
...
### P1-004 app/protected/master-data/actions.ts:156 catches an error and discards it without a word

```
id:         67178d369290
rule:       scan.error-swallowed  [deterministic]  layer: static
surface:    app/protected/master-data/actions.ts:156
expected:   A catch block either handles the error, rethrows it, reports it, or carries a comment saying why ignoring it is correct.
actual:     The catch block is empty and holds no comment. Whatever { revalidateTag("fee-policy", "max"); } throws is discarded, and the code after the try runs as though nothing happened.
source:     try { revalidateTag("fee-policy", "max"); } catch {}
why:        Every empty catch in this repo today wraps a cache bust on a money path — revalidateTag("fee-policy") after a promotion, a fee-setup publish, or a session write. A failed bust that says nothing leaves the Payment Desk pricing off the previous year's schedule for the life of the cache entry, with no error anywhere to connect it to.
...
### P1-005 app/protected/payments/waive-late-fee-actions.ts:34 rounds the amount before Number.isInteger can reject it

```
id:         91c9f99f050e
rule:       scan.money-round-then-validate  [deterministic]  layer: static
surface:    app/protected/payments/waive-late-fee-actions.ts:34
expected:   A fractional rupee entered on a form is refused, because every money column in supabase/migrations is `integer` and the ledger has no way to hold the paise.
actual:     parseAmount() applies Math.round to the parsed value, and line 91 then tests `Number.isInteger(amount)`. That test can never fail — Math.round has already made it an integer — so "1500.75" is silently accepted and posted as 1501.
source:     return Number.isFinite(parsed) ? Math.round(parsed) : NaN;
why:        The guard reads like input validation and is dead code. A parent handed a receipt for a rupee more than they paid has no way to see where it came from, and the ledger carries a figure nobody typed.
fix:        Drop the Math.round from parseAmount() and let Number.isInteger do the job it was written for — the shape already used by parsePaymentAmount in app/protected/payments/actions.ts and parseAmount in app/protected/ledger/actions.ts, both of which validate the raw Number().
```

### P1-006 app/protected/students/[studentId]/page.tsx splits money by installmentCount without preserving the remainder

```
id:         776e64a0e4fc
rule:       scan.money-split-not-conserving  [deterministic]  layer: static
surface:    app/protected/students/[studentId]/page.tsx:509
expected:   A rupee total divided into parts is split by a remainder-preserving splitter, so the parts sum back to the total: splitAmountWithRemainderLast (lib/fees/workbook.ts) sends the leftover to the last part, splitAcrossInstallments (lib/fees/generator.ts) to the first.
actual:     Lines 509, 514 compute Math.round(row.amount / installmentCount) independently per part. installmentCount x Math.round(total / installmentCount) does not equal total whenever the division leaves a remainder: a Rs 19,502 head over 4 installments renders as 4 x Rs 4,876 = Rs 19,504, two rupees more than the head it was split from.
source:     headRows.push({ label: row.label, amount: Math.round(row.amount / installmentCount) });
why:        This is the per-installment breakdown a parent is shown. Rows that do not add to the total printed beside them are the kind of discrepancy the office cannot explain at the counter, and the workbook engine itself never produces them.
fix:        Call splitAmountWithRemainderLast(total, installmentCount) and index into the result, instead of rounding each part separately.
```

### P1-007 app/protected/students/close-due-actions.ts:25 rounds the amount before Number.isInteger can reject it

```
id:         1f934b3f9da3
rule:       scan.money-round-then-validate  [deterministic]  layer: static
surface:    app/protected/students/close-due-actions.ts:25
expected:   A fractional rupee entered on a form is refused, because every money column in supabase/migrations is `integer` and the ledger has no way to hold the paise.
actual:     parseAmount() applies Math.round to the parsed value, and line 51 then tests `Number.isInteger(amount)`. That test can never fail — Math.round has already made it an integer — so "1500.75" is silently accepted and posted as 1501.
source:     return Number.isFinite(parsed) ? Math.round(parsed) : NaN;
why:        The guard reads like input validation and is dead code. A parent handed a receipt for a rupee more than they paid has no way to see where it came from, and the ledger carries a figure nobody typed.
fix:        Drop the Math.round from parseAmount() and let Number.isInteger do the job it was written for — the shape already used by parsePaymentAmount in app/protected/payments/actions.ts and parseAmount in app/protected/ledger/actions.ts, both of which validate the raw Number().
```

### P1-008 components/payments/mobile-payment-flow-sheet.tsx prorates 4 money heads through `proratedShare` and rounds each one separately

```
id:         5a7942426a1a
rule:       scan.money-split-not-conserving  [deterministic]  layer: static
surface:    components/payments/mobile-payment-flow-sheet.tsx:1112
expected:   Prorating a total across a subset of installments produces parts that still sum to the prorated total — the same guarantee splitAmountWithRemainderLast gives.
actual:     `proratedShare` is defined on line 1109 as a fraction over installmentCount, and 4 heads on lines 1112, 1116, 1118, 1121 are each rounded independently against it (Math.round(dist.tuitionFee * proratedShare)). Independent roundings of one ratio compound: the head rows can miss the overdue total shown above them by a rupee per head, in either direction.
source:     { label: "Tuition", amount: Math.round(dist.tuitionFee * proratedShare) },
why:        This is the overdue breakdown on the Payment Desk's mobile flow — the figure a cashier reads out loud while a parent is standing there. The heads and the amount being collected have to agree.
fix:        Derive the prorated heads from the installment charges the workbook engine already produced (buildWorkbookInstallmentCharges in lib/fees/workbook.ts) and sum them, rather than scaling annual heads by a ratio at render time.
```

### P1-009 fast-uri has 1 known high advisory

```
id:         9ed04ce31e3a
rule:       scan.dependency-vulnerable  [deterministic]  layer: static
surface:    package.json:114
expected:   No package in the installed tree carries a high or critical advisory, or the ones that do are recorded with a reason and a date.
actual:     fast-uri is a transitive dependency. Advisories: [high] fast-uri vulnerable to host confusion via backslash authority introducer (https://github.com/advisories/GHSA-7p8r-x3mc-p8w7, affects >=4.0.0 <4.1.2). npm reports the installed range as 4.0.0 - 4.1.1. npm audit reports a fix is available within the current ranges. Marked vulnerable because of it: ajv. No dependency declared in package.json was traceable to it through the audit's effects graph. It is present in the production dependency tree (`npm audit --omit=dev` reports it), so it ships. package.json overrides pins fast-uri to "4.1.1", and the audit still flags the installed version — so `npm audit fix` cannot move this one until that pin changes.
source:     "fast-uri": "4.1.1",
why:        A known advisory is the one class of vulnerability an attacker does not have to find — it is published, with a proof of concept, against a version number anybody can read off a deployment. This app holds every student's fee ledger for a live school, so the question a high advisory raises is not whether the flaw is exploitable in the abstract but whether this code path reaches it, and that question deserves an answer written down rather than left open.
fix:        npm audit reports a fix is available within the current ranges. Take it if it is safe to take; if the fix is semver-major or there is none, decide explicitly whether this code reaches the vulnerable path, and waive it in tests/deep/baseline/known-findings.json with that reasoning and an expiry rather than leaving a P1 permanently red.
```

### P1-010 brace-expansion has 1 known high advisory

```
id:         a0e974411aed
rule:       scan.dependency-vulnerable  [deterministic]  layer: static
surface:    package.json:117
expected:   No package in the installed tree carries a high or critical advisory, or the ones that do are recorded with a reason and a date.
actual:     brace-expansion is a transitive dependency. Advisories: [high] brace-expansion: DoS via unbounded intermediate arrays, bypassing the CVE-2026-14257 mitigation (https://github.com/advisories/GHSA-rgw5-rvv9-x895, affects >=4.0.0 <5.0.9). npm reports the installed range as 4.0.0 - 5.0.8. npm audit's fix is @sentry/nextjs@10.39.0, a semver-major change. Marked vulnerable because of it: minimatch. Reached from this repo's own @sentry/nextjs. It is present in the production dependency tree (`npm audit --omit=dev` reports it), so it ships. package.json overrides pins brace-expansion to "5.0.8", and the audit still flags the installed version — so `npm audit fix` cannot move this one until that pin changes.
source:     "brace-expansion": "1.1.16"
why:        A known advisory is the one class of vulnerability an attacker does not have to find — it is published, with a proof of concept, against a version number anybody can read off a deployment. This app holds every student's fee ledger for a live school, so the question a high advisory raises is not whether the flaw is exploitable in the abstract but whether this code path reaches it, and that question deserves an answer written down rather than left open.
fix:        npm audit's fix is @sentry/nextjs@10.39.0, a semver-major change. Take it if it is safe to take; if the fix is semver-major or there is none, decide explicitly whether this code reaches the vulnerable path, and waive it in tests/deep/baseline/known-findings.json with that reasoning and an expiry rather than leaving a P1 permanently red.
```

### P1-011 ip-address has 1 known high advisory

```
id:         78c6f6d67a50
rule:       scan.dependency-vulnerable  [deterministic]  layer: static
surface:    package.json:57
expected:   No package in the installed tree carries a high or critical advisory, or the ones that do are recorded with a reason and a date.
actual:     ip-address is a transitive dependency. Advisories: [high] ip-address: Address4 decodes leading-zero octets as decimal while resolvers decode them as octal, allowing SSRF and trust-boundary bypass (https://github.com/advisories/GHSA-mwp4-54f8-5fhr, affects <=10.3.0). npm reports the installed range as <=10.3.0. npm audit reports a fix is available within the current ranges. No dependency declared in package.json was traceable to it through the audit's effects graph. It is present in the production dependency tree (`npm audit --omit=dev` reports it), so it ships.
source:     "dependencies": {
why:        A known advisory is the one class of vulnerability an attacker does not have to find — it is published, with a proof of concept, against a version number anybody can read off a deployment. This app holds every student's fee ledger for a live school, so the question a high advisory raises is not whether the flaw is exploitable in the abstract but whether this code path reaches it, and that question deserves an answer written down rather than left open.
fix:        npm audit reports a fix is available within the current ranges. Take it if it is safe to take; if the fix is semver-major or there is none, decide explicitly whether this code reaches the vulnerable path, and waive it in tests/deep/baseline/known-findings.json with that reasoning and an expiry rather than leaving a P1 permanently red.
```

### P1-012 nanoid has 1 known high advisory
...
surface:    package.json:57
expected:   No package in the installed tree carries a high or critical advisory, or the ones that do are recorded with a reason and a date.
actual:     nanoid is a transitive dependency. Advisories: [high] nanoid: custom generators can loop indefinitely when size is zero (https://github.com/advisories/GHSA-2v37-7h3g-55p8, affects <3.3.18). npm reports the installed range as <3.3.18. npm audit reports a fix is available within the current ranges. No dependency declared in package.json was traceable to it through the audit's effects graph. It is present in the production dependency tree (`npm audit --omit=dev` reports it), so it ships.
source:     "dependencies": {
why:        A known advisory is the one class of vulnerability an attacker does not have to find — it is published, with a proof of concept, against a version number anybody can read off a deployment. This app holds every student's fee ledger for a live school, so the question a high advisory raises is not whether the flaw is exploitable in the abstract but whether this code path reaches it, and that question deserves an answer written down rather than left open.
fix:        npm audit reports a fix is available within the current ranges. Take it if it is safe to take; if the fix is semver-major or there is none, decide explicitly whether this code reaches the vulnerable path, and waive it in tests/deep/baseline/known-findings.json with that reasoning and an expiry rather than leaving a P1 permanently red.
```

### P2-013 app/api/cron/nightly-backup/route.ts does bulk work on a schedule and declares no maxDuration

```
id:         8176f6841d11
rule:       scan.config-risk  [deterministic]  layer: static
surface:    app/api/cron/nightly-backup/route.ts:85
expected:   A handler that runs unattended and whose work grows with the data declares its own maxDuration, the way the two admin maintenance routes and every export route do.
actual:     No export const maxDuration in this file, so it takes the platform default of 60s. The handler caps a query with .limit(), so it expects rows in bulk, and uploads to object storage inside the request.
source:     const query = supabase.from(table).select("*").limit(ROW_LIMIT_PER_TABLE[table]);
why:        Nobody is watching a cron. It is scheduled in vercel.json, it runs at night, and when it exceeds its budget the platform kills it mid-flight — here that means a partial set of table dumps under tonight's date prefix, with the manifest that records what succeeded never written, because it is uploaded last. The backup looks present and is incomplete, which is worse than one that is plainly missing.
fix:        Add export const maxDuration = <seconds> (up to 300 on Hobby) to this route, sized against the row caps it already declares.
```

### P2-014 app/protected/dashboard/actions.ts:33 exports repairCurrentSessionDuesAction, which nothing uses
...
### P2-015 app/protected/dashboard/actions.ts:56 exports syncCurrentSessionAction, which nothing uses
...
### P2-016 app/protected/dashboard/actions.ts:69 exports repairPaymentDeskDataAction, which nothing uses
...
### P2-017 app/protected/dashboard/page.tsx:538 formats a rupee figure without lib/helpers/currency.ts
...
### P2-018 app/protected/dashboard/page.tsx:540 formats a rupee figure without lib/helpers/currency.ts
...
### P2-019 app/protected/dashboard/page.tsx:541 formats a rupee figure without lib/helpers/currency.ts
...
### P2-020 app/protected/master-data/actions.ts:224 exports copySessionAction, which nothing uses

```
id:         e31703906f0f
rule:       scan.dead-export  [deterministic]  layer: static
surface:    app/protected/master-data/actions.ts:224
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
...
### P2-021 app/protected/settings/page.tsx:160 formats a rupee figure without lib/helpers/currency.ts
...
### P2-022 app/protected/students/[studentId]/edit/page.tsx:198 formats a rupee figure without lib/helpers/currency.ts
...
### P2-023 app/protected/students/repayment-plan-actions.ts:327 formats a rupee figure without lib/helpers/currency.ts
...
### P2-024 components/auth-button.tsx:7 exports AuthButton, which nothing uses
...
### P2-025 components/dashboard/class-installment-matrix.tsx:13 exports ClassInstallmentMatrixTable, which nothing uses
...
### P2-026 components/dashboard/copy-reminder-button.tsx:12 exports CopyReminderButton, which nothing uses
...
### P2-027 components/dashboard/morning-brief-shelf.tsx:12 exports MorningBriefShelf, which nothing uses
...
### P2-028 components/dashboard/role-focus-actions.tsx:18 exports RoleFocusActions, which nothing uses
...
### P2-029 components/data-table/selection-bar.tsx:24 exports SelectionBar, which nothing uses
...
### P2-030 components/data-table/table-toolbar.tsx:33 exports TableToolbar, which nothing uses
...
### P2-031 components/mobile-app/mobile-option-sheet.tsx:28 exports MobileOptionSheet, which nothing uses
...
### P2-032 components/office/office-ui.tsx:236 exports ClassTabs, which nothing uses
...
### P2-033 components/sign-up-form.tsx:20 exports SignUpForm, which nothing uses
...
### P2-034 components/students/student-form.tsx:597 formats a rupee figure without lib/helpers/currency.ts
...
### P2-035 components/students/student-form.tsx:600 formats a rupee figure without lib/helpers/currency.ts
...
### P2-036 components/transactions/mobile-day-summary.tsx:82 exports todaySummaryDateLabel, which nothing uses
...
### P2-037 components/trust/audit-link.tsx:21 exports AuditLink, which nothing uses
...
### P2-038 components/ui/empty-state.tsx:18 exports EmptyState, which nothing uses
...
### P2-039 components/ui/money-breakdown.tsx:34 exports MoneyBreakdown, which nothing uses
...
### P2-040 hooks/use-action-feedback.ts:44 exports isSuccessStatus, which nothing uses
...
### P2-041 hooks/use-action-feedback.ts:48 exports isErrorStatus, which nothing uses
...
### P2-042 hooks/use-action-feedback.ts:52 exports isWarningStatus, which nothing uses
...
### P2-043 hooks/use-online-status.ts:18 exports useOnlineStatus, which nothing uses
...
### P2-044 lib/activity/events.ts:174 exports getLastEventByRef, which nothing uses
...
### P2-045 lib/activity/events.ts:46 exports activityKindLabel, which nothing uses
...
### P2-046 lib/auth/roles.ts:200 exports isStaffPermission, which nothing uses
...
### P2-047 lib/config/fee-rules.ts:112 exports CORE_FEE_HEADS, which nothing uses
...
### P2-048 lib/config/fee-rules.ts:218 exports buildDefaultInstallmentSchedule, which nothing uses
...
### P2-049 lib/config/fee-rules.ts:240 formats a rupee figure without lib/helpers/currency.ts
...
### P2-050 lib/config/school.ts:18 exports productPrinciples, which nothing uses
...
### P2-051 lib/dashboard/analytics.ts:266 exports emptyDashboardAnalytics, which nothing uses
...
### P2-052 lib/dashboard/anomaly-rules.ts:39 exports evaluateDashboardAnomalies, which nothing uses
...
### P2-053 lib/dashboard/anomaly-rules.ts:54 formats a rupee figure without lib/helpers/currency.ts
...
### P2-054 lib/dashboard/summary.ts:171 formats a rupee figure without lib/helpers/currency.ts
...
### P2-055 lib/data-table/keyboard-nav.ts:27 exports useRowKeyboardNav, which nothing uses
...
### P2-056 lib/defaulters/behavior.ts:84 exports PAYMENT_BEHAVIORS, which nothing uses
...
### P2-057 lib/defaulters/fee-breakdown.ts:176 exports nextFocusInstallment, which nothing uses
...
### P2-058 lib/fees/config-change.ts:1269 exports createConfigChangePreview, which nothing uses
...
### P2-059 lib/fees/config-change.ts:1322 exports applyConfigChangeBatch, which nothing uses
...
### P2-060 lib/fees/config-change.ts:195 formats a rupee figure without lib/helpers/currency.ts
...
### P2-061 lib/fees/config-change.ts:233 formats a rupee figure without lib/helpers/currency.ts
...
### P2-062 lib/fees/config-change.ts:410 formats a rupee figure without lib/helpers/currency.ts
...
### P2-063 lib/fees/config-change.ts:417 formats a rupee figure without lib/helpers/currency.ts
...
### P2-064 lib/fees/config-change.ts:424 formats a rupee figure without lib/helpers/currency.ts
...
### P2-065 lib/fees/config-change.ts:756 formats a rupee figure without lib/helpers/currency.ts
...
### P2-066 lib/fees/policy.ts:2214 exports getAcceptedPaymentModeOptions, which nothing uses
...
### P2-067 lib/fees/policy.ts:290 formats a rupee figure without lib/helpers/currency.ts
...
### P2-068 lib/fees/workbook-setup-change.ts:125 formats a rupee figure without lib/helpers/currency.ts
...
### P2-069 lib/fees/workbook-setup-change.ts:161 formats a rupee figure without lib/helpers/currency.ts
...
### P2-070 lib/fees/workbook-setup-change.ts:404 formats a rupee figure without lib/helpers/currency.ts
...
### P2-071 lib/fees/workbook-setup-change.ts:418 formats a rupee figure without lib/helpers/currency.ts
...
### P2-072 lib/fees/workbook.ts:182 exports getWorkbookStudentStatusLabel, which nothing uses
...
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `getWorkbookStudentStatusLabel` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside lib/fees/workbook.ts, at its own declaration. 8 module(s) import this file, none of them for this name.
source:     export function getWorkbookStudentStatusLabel(studentType: "new" | "existing") {
why:        Dead code is not neutral here. It compiles, it is type-checked, it is counted against the source-line budgets in quality/office-quality-budgets.json, and the next reader takes it for something that runs — so a half-finished extraction reads as a finished one, and a retired module reads as a live one.
fix:        Delete it, or wire up the caller it was written for. If it is deliberately kept as an entry point for something outside this repo's import graph, say so in a comment beside the export — this rule reads the whole tree and will keep reporting it otherwise.
```

### P2-073 lib/fees/workbook.ts:186 exports getWorkbookStudentStatusCode, which nothing uses
...
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `getWorkbookStudentStatusCode` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside lib/fees/workbook.ts, at its own declaration. 8 module(s) import this file, none of them for this name.
source:     export function getWorkbookStudentStatusCode(value: string | null | undefined) {
why:        Dead code is not neutral here. It compiles, it is type-checked, it is counted against the source-line budgets in quality/office-quality-budgets.json, and the next reader takes it for something that runs — so a half-finished extraction reads as a finished one, and a retired module reads as a live one.
fix:        Delete it, or wire up the caller it was written for. If it is deliberately kept as an entry point for something outside this repo's import graph, say so in a comment beside the export — this rule reads the whole tree and will keep reporting it otherwise.
```

### P2-074 lib/helpers/time-ago.ts:5 exports timeAgoShort, which nothing uses
...
### P2-075 lib/import/data.ts:1637 exports getResumableFailedRowCount, which nothing uses
...
### P2-076 lib/import/parser.ts:133 formats an en-IN number without lib/helpers/currency.ts
...
### P2-077 lib/import/parser.ts:145 formats an en-IN number without lib/helpers/currency.ts
...
### P2-078 lib/import/types.ts:317 exports INITIAL_STUDENT_IMPORT_ACTION_STATE, which nothing uses
...
### P2-079 lib/payments/bulk/template.ts:138 formats a rupee figure without lib/helpers/currency.ts
...
### P2-080 lib/payments/data.ts:239 formats a rupee figure without lib/helpers/currency.ts
...
### P2-081 lib/payments/payment-desk-client-helpers.ts:29 exports sanitizeWholeInput, which nothing uses
...
### P2-082 lib/payments/payment-desk-workflow.ts:100 formats a rupee figure without lib/helpers/currency.ts
...
### P2-083 lib/pdf/document-kit.tsx:52 formats a rupee figure without lib/helpers/currency.ts
...
### P2-084 lib/prev-year-dues/constants.ts:31 exports CARRY_FORWARD_ROLLBACK_HINT, which nothing uses
...
### P2-085 lib/promotion/data.ts:834 formats a rupee figure without lib/helpers/currency.ts
...
### P2-086 lib/promotion/data.ts:975 formats a rupee figure without lib/helpers/currency.ts
...
### P2-087 lib/repayment-plans/schedule.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-088 lib/setup/data.ts:271 formats a rupee figure without lib/helpers/currency.ts
...
### P2-089 lib/setup/data.ts:603 exports saveSetupPolicy, which nothing uses
...
### P2-090 lib/setup/data.ts:630 exports saveSetupSchoolDefaults, which nothing uses
...
### P2-091 lib/setup/data.ts:647 exports saveSetupClasses, which nothing uses
...
### P2-092 lib/setup/data.ts:748 exports saveSetupRoutes, which nothing uses
...
### P2-093 lib/setup/data.ts:816 exports saveSetupClassDefaults, which nothing uses
...
### P2-094 lib/setup/data.ts:848 exports markSetupStageComplete, which nothing uses
...
### P2-095 lib/setup/types.ts:11 exports INITIAL_SETUP_ACTION_STATE, which nothing uses
...
### P2-096 lib/staff-management/data.ts:531 exports provisionBootstrapStaff, which nothing uses
...
### P2-097 lib/students/bulk-update/fields.ts:306 exports bulkUpdateFieldsAffectFees, which nothing uses
...
### P2-098 lib/students/constants.ts:16 exports STUDENT_IMPORT_FIELDS, which nothing uses
...
### P2-099 lib/students/populations.ts:48 exports RECOVERY_STATUSES, which nothing uses
...
### P2-100 lib/system-sync/finance-sync.ts:43 exports syncStudentDues, which nothing uses
...
### P2-101 lib/system-sync/finance-sync.ts:50 exports syncStudentDuesAsSystem, which nothing uses
...
### P2-102 lib/system-sync/finance-sync.ts:85 exports syncAfterBulkStudentImport, which nothing uses
...
### P2-103 lib/system-sync/finance-sync.ts:93 exports syncAfterFeeSetupPublish, which nothing uses
...
### P2-104 lib/whatsapp-templates/data.ts:60 exports getWhatsappTemplate, which nothing uses
...
### P2-105 No security response header is configured for any document response
...
### P2-106 xlsx is installed from a URL, so npm audit cannot see it

```
id:         01916769d1ca
rule:       scan.config-risk  [deterministic]  layer: static
surface:    package.json:81
...
### P2-107 app/protected/students/close-due-actions.ts:30 rounds `value` where the domain core truncates it

```
id:         4c2c39ae9065
rule:       scan.rounding-policy-mixed  [heuristic]  layer: static
surface:    app/protected/students/close-due-actions.ts:30
...
actual:     This coerces `value` with Math.round, while 9 other sites coerce the same-named quantity with Math.trunc (lib/fees/conventional-discount-rules.ts:16, lib/fees/conventional-discounts.ts:167, lib/fees/due-amounts.ts:21). For 1500.6 one answers 1501 and the other 1500.
...
### P2-108 lib/helpers/amount-in-words-hi.ts:73 rounds `value` where the domain core truncates it
...
### P2-109 lib/helpers/amount-in-words.ts:84 rounds `value` where the domain core truncates it
...
### P2-110 lib/pdf/document-kit.tsx:51 rounds `value` where the domain core truncates it
...
### P2-111 private.enforce_max_active_conventional_discounts is defined without a pinned search_path
...
### P2-112 private.prevent_receipt_adjustment_mutation is defined without a pinned search_path
...
### P2-113 private.enforce_max_active_conventional_discounts_in_schema is defined without a pinned search_path
...
### P2-114 private.enforce_third_child_traceability is defined without a pinned search_path
...
### P2-115 private.derive_family_child_client_request_id is defined without a pinned search_path
...
### P2-116 public.refresh_financial_materialized_views is defined without a pinned search_path
...
### P2-117 private.prevent_notion_sync_log_mutation is defined without a pinned search_path
...
### P2-118 public.v_notion_student_fee_summary does not set security_invoker
...
### P2-119 public.v_notion_daily_collection_summary does not set security_invoker
...
### P2-120 public.v_notion_family_fee_summary does not set security_invoker
...
### P3-121 app/protected/admin-tools/session-health/session-health-grid.tsx hardcodes the live session 2026-27
...
### P3-122 app/protected/exports/page.tsx declares revalidate = 60, which never takes effect
...
### P3-123 app/protected/fee-setup/page.tsx declares revalidate = 60, which never takes effect
...
### P3-124 app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27

```
id:         9095bbcc2aca
rule:       scan.observation  [heuristic]  layer: static
surface:    app/protected/students/[studentId]/page.tsx:1195
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
...
### P3-125 app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27

```
id:         ead8db54893f
rule:       scan.observation  [heuristic]  layer: static
surface:    app/protected/students/[studentId]/page.tsx:1334
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
actual:     "2026-27" appears as a fallback for a value read at runtime. Nothing in this module writes through a Supabase client and it is not a "use server" action, so no ledger is at risk — this is inventory of where the live year is baked in, not a defect.
source:     sessionLabel={financialSnapshot?.policy.academicSessionLabel || "2026-27"}
why:        Every hardcoded copy is a place the AY 2027-28 rollover has to find. It also quietly decides for a reader which session they are looking at: a fallback or an is-current comparison that names 2026-27 keeps saying so after the school has moved on.
...
### P3-126 pending-late-fee: the two sides are pinned, and they already disagree

```
id:         23b191f743c8
rule:       scan.observation  [heuristic]  layer: static
surface:    lib/fees/due-amounts.ts:40
expected:   The late fee a student still owes for the session, summed across installments, net of waivers and of any payment that already covered it. Declared at supabase/migrations/20260809110000_late_fee_waived_means_manually_waived.sql:54 ("Mirrors calculatePendingLateFeeAmount (lib/fees/due-amounts.ts:40-45)").
actual:     The SQL side now reads late_fee_pending; the TypeScript still computes min(finalLateFee, outstandingAmount ?? pendingAmount). Callers that pass the fee-only `pending_amount` column into it therefore read 0 for a family whose fees are clear and whose late fee is not -- lib/students/data.ts:838 passes exactly that. The Payment Desk path is unaffected because lib/payments/data.ts:2120 maps outstandingAmount to total_pending, which still includes the late fee.
source:     export function calculatePendingLateFeeAmount(rows: readonly InstallmentLike[]) {
why:        A pinned hash proves that neither side has moved since it was pinned. It cannot prove they agreed when they were pinned — this check reads text, not semantics. Recording the known divergence keeps a silent manifest from reading as a clean bill of health.
fix:        Bring the TypeScript in line with the migration, re-pin, and drop the `diverged` note from the pair in tests/scan/checks/mirror-drift.mjs.
```

### P3-127 lib/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-128 lib/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-129 workers/schoolfees-mcp/src/prompts.mjs hardcodes the live session 2026-27
...
### P3-130 workers/schoolfees-mcp/src/toolkit.mjs hardcodes the live session 2026-27
...
### P3-131 workers/schoolfees-mcp/src/tools/orientation.mjs hardcodes the live session 2026-27
...
### P3-132 workers/schoolfees-mcp/worker.mjs hardcodes the live session 2026-27
...
| Rule | Severity | Count |
|---|---|---:|
| `scan.error-swallowed` | P1 | 4 |
| `scan.dependency-vulnerable` | P1 | 4 |
| `scan.money-round-then-validate` | P1 | 2 |
| `scan.money-split-not-conserving` | P1 | 2 |
| `scan.dead-export` | P2 | 59 / 59 |
| `scan.money-format-raw` | P2 | 32 / 32 |
| `scan.sql-risk` | P2 | 10 / 10 |
| `scan.rounding-policy-mixed` | P2 | 4 / 4 |
| `scan.config-risk` | P2 | 3 / 2 |
| `scan.observation` | P3 | 12 / 12 |
