...
| Run id | `202609100525` |
| Layers | static |
| Git | `5d5f5bfb3b04` on main (dirty) |
| Node | v24.19.0 |
| Files read | 1615 |
| Started | 2026-09-10T05:25:03.123Z |
| Duration | 6651 ms |
| Strict gate | no |
...
10 check(s) swept 15029 of 15029 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 122 | 122 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 769 | 769 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 769 | 769 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 769 | 769 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 737 | 737 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 232 | 232 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1432 | 1432 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7713 | 7713 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2475 | 2475 | exhaustive |
