...
| Run id | `202609040319` |
| Layers | static |
| Git | `4c4940feba5b` on main (dirty) |
| Node | v26.7.0 |
| Files read | 1602 |
| Started | 2026-09-04T03:19:55.415Z |
| Duration | 185767 ms |
| Strict gate | no |
...
11 check(s) swept 14833 of 14833 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 120 | 120 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 752 | 752 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 752 | 752 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 752 | 752 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 719 | 719 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 227 | 227 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1406 | 1406 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7730 | 7730 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2364 | 2364 | exhaustive |
| `config-risk` | deployment and build configuration candidates | 11 | 11 | exhaustive |
| `deps` | npm advisories across the installed dependency tree | 0 | 0 | errored |
...
- **deps** — THE DEPENDENCY AUDIT DID NOT RUN. npm audit did not finish within 180s — usually a registry that is unreachable rather than slow. No scan.dependency-vulnerable finding was produced, and that must not be read as a clean tree — nothing was checked. The structural half of this check did run: 1 dependency(ies) specified outside the registry were reported as scan.config-risk. Re-run with network access to the npm registry, or run `npm audit` by hand, before treating dependency risk as assessed.
...
### P2-010 src/modules/dashboard/domain/summary.ts:171 formats a rupee figure without lib/helpers/currency.ts
...
### P2-019 src/modules/payments/data/queries.ts:239 formats a rupee figure without lib/helpers/currency.ts
...
### P2-024 src/modules/repayment-plans/domain/schedule.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-025 src/modules/students/ui/student-form.tsx:609 formats a rupee figure without lib/helpers/currency.ts
...
### P2-026 src/modules/students/ui/student-form.tsx:612 formats a rupee figure without lib/helpers/currency.ts
...
### P2-027 src/modules/whatsapp/data/delivery-store.ts:223 exports loadSeenButNotPaid, which nothing uses
...
### P2-028 src/platform/config/fee-rules.ts:180 formats a rupee figure without lib/helpers/currency.ts
...
### P2-029 src/platform/helpers/date.ts:246 exports istTodayIso, which nothing uses
...
### P2-030 src/platform/pdf/document-kit.tsx:52 formats a rupee figure without lib/helpers/currency.ts
...
### P2-034 src/platform/pdf/document-kit.tsx:51 rounds `value` where the domain core truncates it
