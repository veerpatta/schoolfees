# Source scan 202609130225

**FAIL** · 59 finding(s) · P0 0 · P1 0 · P2 47 · P3 12
...
| Node | v24.19.0 |
| Files read | 1617 |
| Started | 2026-09-13T02:25:13.187Z |
| Duration | 25363 ms |
| Strict gate | no |

## Verdict

The gate **failed** for 3 reason(s):

- scan.money-format-raw regressed: 26 findings against a baseline budget of 25.
- scan.rounding-policy-mixed regressed: 5 findings against a baseline budget of 4.
- scan.observation regressed: 12 findings against a baseline budget of 11.

## What this scan did NOT look at

10 check(s) swept 15009 of 15009 enumerated members.

| Check | Dimension | Domain | Examined | Strategy |
|---|---|---:|---:|---|
| `guards` | app route handlers, pages and server-action modules | 124 | 124 | exhaustive |
| `client-boundary` | source modules crossed against the client bundle import graph | 774 | 774 | exhaustive |
| `session-safety` | product source modules scanned for hardcoded academic-session labels | 774 | 774 | exhaustive |
| `money` | product source files (app, components, lib, workers, hooks, i18n; no tests, no scripts) | 774 | 774 | exhaustive |
| `async-safety` | product .ts/.tsx modules parsed by the TypeScript checker | 742 | 742 | exhaustive |
| `mirror-drift` | declared TS/SQL mirror sides, plus migrations added since the last reconcile | 236 | 236 | targeted |
| `sql-safety` | supabase migrations, plus the source files that build SQL text | 1450 | 1450 | exhaustive |
| `i18n` | locale catalogue keys (union x locale) and t() call sites in product source | 7601 | 7601 | exhaustive |
| `dead-code` | export declarations in app/, components/, lib/, hooks/ and i18n/ | 2523 | 2523 | exhaustive |
...
### P2-006 src/app/protected/defaulters/actions.ts:255 exports logWhatsAppSendAttempts, which nothing uses
...
### P2-007 src/app/protected/settings/page.tsx:160 formats a rupee figure without lib/helpers/currency.ts
...
### P2-008 src/app/protected/students/[studentId]/edit/page.tsx:302 formats a rupee figure without lib/helpers/currency.ts
...
### P2-009 src/app/protected/students/repayment-plan-actions.ts:327 formats a rupee figure without lib/helpers/currency.ts
...
### P2-010 src/modules/dashboard/domain/summary.ts:162 formats a rupee figure without lib/helpers/currency.ts
...
### P2-016 src/modules/fees/domain/policy-shaping.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-017 src/modules/imports/domain/parser.ts:133 formats an en-IN number without lib/helpers/currency.ts
...
### P2-018 src/modules/imports/domain/parser.ts:145 formats an en-IN number without lib/helpers/currency.ts
...
### P2-019 src/modules/payments/data/queries.ts:242 formats a rupee figure without lib/helpers/currency.ts
...
### P2-020 src/modules/payments/domain/bulk/template.ts:138 formats a rupee figure without lib/helpers/currency.ts
...
### P2-021 src/modules/payments/domain/payment-desk-workflow.ts:100 formats a rupee figure without lib/helpers/currency.ts
...
### P2-022 src/modules/promotion/data/queries.ts:834 formats a rupee figure without lib/helpers/currency.ts
...
### P2-023 src/modules/promotion/data/queries.ts:975 formats a rupee figure without lib/helpers/currency.ts
...
### P2-024 src/modules/receipts/domain/share-docs.ts:26 exports buildReceiptShareDocs, which nothing uses
...
### P2-025 src/modules/repayment-plans/domain/schedule.ts:154 formats a rupee figure without lib/helpers/currency.ts
...
### P2-026 src/modules/students/ui/student-form.tsx:609 formats a rupee figure without lib/helpers/currency.ts
...
### P2-027 src/modules/students/ui/student-form.tsx:612 formats a rupee figure without lib/helpers/currency.ts
...
### P2-028 src/modules/whatsapp/data/delivery-store.ts:223 exports loadSeenButNotPaid, which nothing uses
...
### P2-029 src/platform/config/fee-rules.ts:180 formats a rupee figure without lib/helpers/currency.ts
...
### P2-030 src/platform/pdf/document-kit.tsx:129 exports renderQrDataUri, which nothing uses

```
id:         5aed1f2cf21e
rule:       scan.dead-export  [deterministic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:129
expected:   Every exported value is either imported somewhere, resolved by Next.js by convention, or deleted.
actual:     `renderQrDataUri` is exported and the identifier appears in no other source file in the repository — not in app/, components/, lib/, hooks/, tests/ or scripts/ — and only once inside src/platform/pdf/document-kit.tsx, at its own declaration. 3 module(s) import this file, none of them for this name.
source:     export async function renderQrDataUri(url: string | null): Promise<string | null> {
why:        Dead code is not neutral here. It compiles, it is type-checked, it is counted against the source-line budgets in quality/office-quality-budgets.json, and the next reader takes it for something that runs — so a half-finished extraction reads as a finished one, and a retired module reads as a live one.
fix:        Delete it, or wire up the caller it was written for. If it is deliberately kept as an entry point for something outside this repo's import graph, say so in a comment beside the export — this rule reads the whole tree and will keep reporting it otherwise.
```

### P2-031 src/platform/pdf/document-kit.tsx:56 formats a rupee figure without lib/helpers/currency.ts

```
id:         8638de9627df
rule:       scan.money-format-raw  [deterministic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:56
expected:   Every rupee a person reads is produced by formatInr() or <Money />, and every other en-IN grouped number by the plain formatter beside it, so a find-references on src/platform/helpers/currency.ts reaches every one of them.
actual:     This line uses toLocaleString("en-IN") directly. scripts/audit-money-formatting.mjs enforces the same rule, but only walks app/ and components/ — it never reads src/.
...
### P2-032 src/platform/pdf/document-kit.tsx:76 formats a rupee figure without lib/helpers/currency.ts

```
id:         9b482cbe7fe8
rule:       scan.money-format-raw  [deterministic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:76
expected:   Every rupee a person reads is produced by formatInr() or <Money />, and every other en-IN grouped number by the plain formatter beside it, so a find-references on src/platform/helpers/currency.ts reaches every one of them.
actual:     This line uses toLocaleString("en-IN") directly. scripts/audit-money-formatting.mjs enforces the same rule, but only walks app/ and components/ — it never reads src/.
source:     return `₹${rounded.toLocaleString("en-IN")}`;
why:        The point of the rule is grep-ability, not aesthetics: null, zero, sign, paise and the symbol are decided once in formatInr. A second formatter is a second set of answers, and nobody finds it when the first one changes.
fix:        Use formatInr() (or <Money /> in JSX). If the divergence is deliberate — react-pdf's Helvetica genuinely has no ₹ glyph — put the reason on the line with `// @allow-raw-money-format`, which both this check and the audit script honour.
```

### P2-033 src/app/protected/students/close-due-actions.ts:47 rounds `value` where the domain core truncates it
...
### P2-034 src/platform/helpers/amount-in-words-hi.ts:73 rounds `value` where the domain core truncates it
...
### P2-035 src/platform/helpers/amount-in-words.ts:84 rounds `value` where the domain core truncates it
...
### P2-036 src/platform/pdf/document-kit.tsx:53 rounds `value` where the domain core truncates it

```
id:         cf79c76d0ef0
rule:       scan.rounding-policy-mixed  [heuristic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:53
expected:   One rounding policy per rupee. The domain core coerces a money value to whole rupees with Math.trunc — lib/fees/due-amounts.ts, lib/receipts/amounts.ts, src/lib/finance/financial-state.ts and lib/payments/allocation.ts all do — so a figure reaches the ledger, the receipt and the export with the same value.
actual:     This coerces `value` with Math.round, while 10 other sites coerce the same-named quantity with Math.trunc (src/modules/fees/data/conventional-discounts.ts:167, src/modules/fees/domain/conventional-discount-rules.ts:16, src/modules/fees/domain/due-amounts.ts:36). For 1500.6 one answers 1501 and the other 1500.
...
### P2-037 src/platform/pdf/document-kit.tsx:74 rounds `value` where the domain core truncates it

```
id:         7c20e574df8a
rule:       scan.rounding-policy-mixed  [heuristic]  layer: static
surface:    src/platform/pdf/document-kit.tsx:74
expected:   One rounding policy per rupee. The domain core coerces a money value to whole rupees with Math.trunc — lib/fees/due-amounts.ts, lib/receipts/amounts.ts, src/lib/finance/financial-state.ts and lib/payments/allocation.ts all do — so a figure reaches the ledger, the receipt and the export with the same value.
actual:     This coerces `value` with Math.round, while 10 other sites coerce the same-named quantity with Math.trunc (src/modules/fees/data/conventional-discounts.ts:167, src/modules/fees/domain/conventional-discount-rules.ts:16, src/modules/fees/domain/due-amounts.ts:36). For 1500.6 one answers 1501 and the other 1500.
source:     const rounded = Math.round(value || 0);
why:        Latent today and not later: every money column in supabase/migrations is `integer`, so both policies agree on everything currently in the database. The first fractional value that reaches one of these — an imported spreadsheet, a percentage discount, a future decimal column — makes the receipt and the ledger differ by a rupee, with nothing in either to say which is right.
...
### P2-038 private.enforce_max_active_conventional_discounts is defined without a pinned search_path
...
### P2-039 private.prevent_receipt_adjustment_mutation is defined without a pinned search_path
...
### P2-040 private.enforce_max_active_conventional_discounts_in_schema is defined without a pinned search_path
...
### P2-041 private.enforce_third_child_traceability is defined without a pinned search_path
...
### P2-042 private.derive_family_child_client_request_id is defined without a pinned search_path
...
### P2-043 public.refresh_financial_materialized_views is defined without a pinned search_path
...
### P2-044 private.prevent_notion_sync_log_mutation is defined without a pinned search_path
...
### P2-045 public.v_notion_student_fee_summary does not set security_invoker
...
### P2-046 public.v_notion_daily_collection_summary does not set security_invoker
...
### P2-047 public.v_notion_family_fee_summary does not set security_invoker
...
### P3-048 src/app/protected/admin-tools/session-health/session-health-grid.tsx hardcodes the live session 2026-27
...
### P3-049 src/app/protected/exports/page.tsx declares revalidate = 60, which never takes effect
...
### P3-050 src/app/protected/fee-setup/page.tsx declares revalidate = 60, which never takes effect
...
### P3-051 src/app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27

```
id:         d9692909a558
rule:       scan.observation  [heuristic]  layer: static
surface:    src/app/protected/students/[studentId]/page.tsx:1404
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
...
### P3-052 src/app/protected/students/[studentId]/page.tsx hardcodes the live session 2026-27

```
id:         c03c145a672b
rule:       scan.observation  [heuristic]  layer: static
surface:    src/app/protected/students/[studentId]/page.tsx:1546
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
actual:     "2026-27" appears as a fallback for a value read at runtime. Nothing in this module writes through a Supabase client and it is not a "use server" action, so no ledger is at risk — this is inventory of where the live year is baked in, not a defect.
source:     sessionLabel={financialSnapshot?.policy.academicSessionLabel || "2026-27"}
why:        Every hardcoded copy is a place the AY 2027-28 rollover has to find. It also quietly decides for a reader which session they are looking at: a fallback or an is-current comparison that names 2026-27 keeps saying so after the school has moved on.
...
### P3-053 src/modules/whatsapp/domain/campaign-bodies-v5.ts hardcodes the live session 2026-27

```
id:         f75bb4896d5a
rule:       scan.observation  [heuristic]  layer: static
surface:    src/modules/whatsapp/domain/campaign-bodies-v5.ts:161
expected:   The live session label is written down in as few places as possible, so that the next rollover is a data change rather than a code change.
actual:     "2026-27" appears as a literal. Nothing in this module writes through a Supabase client and it is not a "use server" action, so no ledger is at risk — this is inventory of where the live year is baked in, not a defect.
source:     sessionLabel: "2026-27",
why:        Every hardcoded copy is a place the AY 2027-28 rollover has to find. It also quietly decides for a reader which session they are looking at: a fallback or an is-current comparison that names 2026-27 keeps saying so after the school has moved on.
fix:        Prefer the resolved session — getActiveSessionLabel(), the switcher's value, or FALLBACK_OFFICE_SESSION_LABEL in lib/session/available-sessions.ts, which is the one place this label is meant to live.
```

### P3-054 src/platform/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-055 src/platform/session/available-sessions.ts hardcodes the live session 2026-27
...
### P3-056 workers/schoolfees-mcp/src/prompts.mjs hardcodes the live session 2026-27
...
### P3-057 workers/schoolfees-mcp/src/toolkit.mjs hardcodes the live session 2026-27
...
### P3-058 workers/schoolfees-mcp/src/tools/orientation.mjs hardcodes the live session 2026-27
...
### P3-059 workers/schoolfees-mcp/worker.mjs hardcodes the live session 2026-27
...
| Rule | Severity | Count |
|---|---|---:|
| `scan.money-format-raw` | P2 | 26 / 25 |
| `scan.sql-risk` | P2 | 10 / 10 |
| `scan.rounding-policy-mixed` | P2 | 5 / 4 |
| `scan.dead-export` | P2 | 4 |
| `scan.config-risk` | P2 | 2 / 3 |
| `scan.observation` | P3 | 12 / 11 |
