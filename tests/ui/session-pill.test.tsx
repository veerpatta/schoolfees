...
describe("SessionPill", () => {
  ...
  it("renders a production session without the TEST tag", async () => {
    ...
    expect(groupSessions(sessions).map((group) => group.title)).toContain("Active");
  });
  ...
  it("renders an archived production session as Other production", async () => {
    ...
    expect(groupSessions(sessions).map((group) => group.title)).toContain("Other production");
  ...
  it("renders TEST sessions with a magenta border and tag", async () => {
    ...
    expect(groupSessions(sessions).map((group) => group.title)).toContain("Test / UAT / DEMO");
  });
});
