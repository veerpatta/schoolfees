...
describe("receipt print and loading UX", () => {
  it("prints the receipt as a full A4 page with the rich fee detail visible", () => {
    ...
    const receiptDocument = readRepoFile("src/modules/receipts/ui/receipt-document-v2.tsx");
  ...
  it("uses a stacked mobile receipt layout while keeping the print table", () => {
    const successSheet = readRepoFile("src/modules/payments/ui/success-receipt-sheet.tsx");
  ...
  it("supports print-ready receipt links from Payment Desk success", () => {
    ...
    const printActions = readRepoFile("src/modules/receipts/ui/receipt-print-actions.tsx");
