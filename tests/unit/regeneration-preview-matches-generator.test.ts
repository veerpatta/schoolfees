 ...
 * the same inputs.
 *
 * `lib/fees/regeneration.ts` computes what the office is shown; the plan it
 * approves is then applied by `lib/fees/generator.ts`. They call the same
...
describe("the Fee Setup preview resolves policy the same way the apply does", () => {
  it("both pass conventionalDiscountAssignments to the resolver", () => {
    for (const file of ["lib/fees/regeneration.ts", "lib/fees/generator.ts"]) {
  ...
  it("both read the same four policy inputs", () => {
    const preview = resolverCall(read("lib/fees/regeneration.ts"));
    const apply = resolverCall(read("lib/fees/generator.ts"));
  ...
  it("the preview allows a rise on an unsettled row, exactly as the generator does", () => {
    ...
    const preview = read("lib/fees/regeneration.ts");

    expect(preview).toContain("appliedAmount < amountDue");
    expect(preview).toContain("charge_rise_on_unsettled");
    expect(read("lib/fees/generator.ts")).toContain('kind: "safe_increase"');
  });
});
