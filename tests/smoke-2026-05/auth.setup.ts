 ...
 *    app's own `/auth/confirm` route exchanges it for a session — the same code
 *    path a staff member takes clicking an emailed link. No password exists in
 *    this flow to be typed, stored or leaked, which is why it is the default.
 * 2. **A password**, read from the environment, if one was supplied.
 *
 * Either way nothing lands in the repo: `tests/smoke-2026-05/.auth/` is
 * gitignored.
 */

const roles = availableRoles();

// With no way in at all the loop below defines zero tests, the setup project
...
/**
 * Exchange a one-time magic-link token for a browser session.
 *
 * `generateLink` returns a token without sending any email, so this works on a
 * project with email delivery switched off. The token is single-use and expires
 * on its own; it never touches disk.
 */
async function mintSession(page: Page, role: SmokeRole) {
  ...
  if (!tokenHash) {
    throw new Error(`No token returned for ${role.email}.`);
  }
...
test.describe("authenticate QA staff logins", () => {
  for (const role of roles) {
    test(`sign in as ${role.role} (${role.key})`, async ({ page }) => {
      ...
      // somewhere else is a real finding, but it must not stop the other four
      // from being captured, so it is reported rather than thrown.
