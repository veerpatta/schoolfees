...
- **Fee Setup publish previews impact first**, and protects paid / partial / adjusted rows from silent rewrite. It leaves carry-forward rows and EMI-covered installments alone. The live engine is `data/workbook-setup-change.ts`.
- **A late fee is not a fee.** `pending_amount` never contains one. The rule is written twice — in `v_workbook_installment_balances` and `private.workbook_installment_snapshot` — and the two are edited together or not at all.
