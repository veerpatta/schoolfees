...
import { PostingReceiptSheet } from "@/modules/payments/ui/posting-receipt-sheet";
import { usePostingIndicator } from "@/ui/hooks/use-posting-indicator";
...
import { OfficeRecentTracker, ValueStatePill, WorkflowGuard } from "@/ui/office/office-ui";
...
import { pushOptimisticPayment } from "@/modules/dashboard/domain/optimistic-counters";
...
import { DeskTotalsSection } from "@/modules/payments/ui/desk-totals-section";
import { CollectDraftBanner } from "@/modules/payments/ui/collect-draft-banner";
import {
  ...
  PaymentDeskRoot,
} from "@/modules/payments/ui/payment-desk/payment-desk-layout";
import { ActionNotice } from "@/modules/payments/ui/payment-desk/action-notice";
import { useMediaQuery } from "@/ui/hooks/use-media-query";
import { useScrollIntoView } from "@/ui/hooks/use-scroll-into-view";
import { useStoredPreference } from "@/ui/hooks/use-stored-preference";
import { buildPaymentAllocation, buildReceiptPreviewAllocation } from "@/modules/payments/domain/allocation";
import { buildPaymentQuickAmounts } from "@/modules/payments/domain/workflow";
import {
...
import { triggerHaptic } from "@/ui/hooks/use-haptics";
import { useOverlayScrollLock } from "@/ui/hooks/use-overlay-scroll-lock";
import { formatInr } from "@/platform/helpers/currency";
import { cn } from "@/platform/utils";
import { clearDraft, loadDraft, saveDraft } from "@/modules/payments/domain/draft-store";
import { normalizeAmountInputShorthand, sanitizeDecimalInput } from "@/modules/payments/domain/payment-desk-client-helpers";
import { getOfficeMetricSessionKind, recordOfficeMetric } from "@/platform/telemetry/office-telemetry";
import {
  markPaymentDeskStudentTiming,
  recordPaymentPostResult,
} from "@/modules/payments/ui/payment-desk-metrics";

const ConfirmReceiptSheet = dynamic(
  () => import("@/modules/payments/ui/confirm-receipt-sheet").then((mod) => mod.ConfirmReceiptSheet),
  { ssr: false },
);

const SuccessReceiptSheet = dynamic(
  () => import("@/modules/payments/ui/success-receipt-sheet").then((mod) => mod.SuccessReceiptSheet),
  { ssr: false },
);
...
const DuplicateReceiptSheet = dynamic(
  () => import("@/modules/payments/ui/duplicate-receipt-sheet").then((mod) => mod.DuplicateReceiptSheet),
  { ssr: false },
);

const MobilePaymentFlowSheet = dynamic(
  () => import("@/modules/payments/ui/mobile-payment-flow-sheet").then((mod) => mod.MobilePaymentFlowSheet),
  { ssr: false },
);
...
export function PaymentDeskClient({
  ...
  useEffect(() => {
    ...
    if (state.status === "success") {
      ...
      // The URL is deliberately NOT touched here. Dropping ?studentId= at this
      // instant is what destroyed the receipt screen: the Server Action's
      // revalidation re-renders this route, the refresh reads the URL we just
      // rewrote, the page falls to its no-student branch and React remounts
      // the whole desk — taking the success state, and the receipt, with it.
      // The clerk saw a flash and landed back on the picker.
      // `handleCollectAnotherPayment` strips it when they move on, which is
      // when a fresh render is wanted. See tests/ui/payment-desk-remount.test.ts.
  ...
  function selectStudent(studentId: string) {
    ...
    void import("@/modules/payments/ui/success-receipt-sheet");
    recordOfficeMetric({
      area: "payment-desk",
