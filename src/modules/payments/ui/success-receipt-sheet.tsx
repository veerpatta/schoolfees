...
import { MobileStepRail } from "@/ui/mobile/mobile-kit";
...
import { SuccessCheckMark } from "@/modules/payments/ui/success-check-mark";
import { formatInr } from "@/platform/helpers/currency";
import { useReducedMotion } from "@/ui/hooks/use-reduced-motion";
import { UNDO_WINDOW_SECONDS } from "@/modules/receipts/domain/undo-window";
      ...
      >
        ...
        <div className="sticky bottom-0 z-10 mt-5 border-t border-border bg-card pt-3 pb-2 mobile-safe-bottom-padding max-md:static max-md:mt-0 max-md:flex-none max-md:border-nav-border max-md:bg-nav max-md:px-4 max-md:pb-[calc(0.75rem+var(--mobile-safe-area-bottom))] sm:pb-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* Phone: one tap attaches the receipt itself. The old anchor sent
                text alone and left staff to fetch and attach the card by hand
                from the More drawer below. Kept for desk, where there is no
                native file share to hand off to. */}
            {whatsappHref && receiptId ? (
              <button
                type="button"
                onClick={() => setShareOpen(true)}
                className={cn(
                  "flex h-9 items-center justify-center gap-2 rounded-md border border-success/30 bg-success-soft px-3.5 text-sm font-semibold text-success-soft-foreground transition-colors hover:bg-success-soft/80 md:hidden max-md:h-11 max-md:px-4",
        ...
        {/* Last child of the dialog, outside the scroll region and outside every
            max-md:/md: branch, so the sheet it opens is never inside a
            display:none subtree.

            canSendReceiptPdf is hardcoded rather than threaded from the server:
            reaching this screen requires `payments:write`, which only admin and
            accountant hold, and both of them also hold `receipts:print`.
            tests/unit/share-doc-permissions.test.ts pins that invariant, so a
            future role change fails a test instead of 403-ing at the counter.
            The alternative — a new prop — costs lines in payment-desk-mobile.tsx,
            which sits at 3515 of its 3520-line budget. */}
        {/* Mounted only once opened. This sheet is deliberately provider-free
            English (see the step-rail note above) and its tests render it bare,
            while the share sheet is translated and needs NextIntlClientProvider
            — which the real app always has and those tests do not. */}
        {receiptId && shareOpen ? (
          <ShareReceiptWhatsApp
            ...
            receipt={{
              ...
              fatherPhone: whatsappPhone ?? null,
              // Undoing leaves this screen open. A receipt that has just been
              // reversed is not a payment confirmation, so the sheet switches
              // to the reversal notice rather than carrying on cheerfully.
              isVoided: undoState === "done",
            }}
          />
        ) : null}
      </div>
