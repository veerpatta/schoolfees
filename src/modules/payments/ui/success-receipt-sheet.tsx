      ...
      >
        ...
        <div className="sticky bottom-0 z-10 mt-5 border-t border-border bg-card pt-3 pb-2 mobile-safe-bottom-padding max-md:static max-md:mt-0 max-md:flex-none max-md:border-nav-border max-md:bg-nav max-md:px-4 max-md:pb-[calc(0.75rem+var(--mobile-safe-area-bottom))] sm:pb-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* The three WhatsApp exits that used to sit here are gone.

                Two of them opened `wa.me` with pre-written text and left the
                receipt itself to be fetched and attached by hand; the third
                copied that text to the clipboard. None of them recorded that a
                parent had been told anything.

                Posting now sends the receipt PDF by itself, from the school's
                own number, in the action's `after()` — so by the time this
                screen renders the message has already gone. Nothing here needs
                to ask. Open Receipt below reaches the one-tap send for the
                cases where it did not (a family with no number on file, a
                provider failure). */}
