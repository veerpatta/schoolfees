...
import { withStudentInfoColumns } from "@/modules/exports/data/responses";
...
import type { StudentInfoRow } from "@/modules/students/domain/info-fields";
import {
...
export async function aiContextBundleResponse(filename: string, sessionLabel: string) {
  ...
  const readmeLines: string[] = [
    ...
    `Installments."Paid" is where the family's money SITS: every rupee paid in`,
    `the session is one pool that clears installment 1, then 2, then 3, then 4`,
    `(each row's fees first, then its late fee), whatever installment the receipt`,
    `was written against. Installments."Receipted here" is that original pin.`,
  ...
  XLSX.utils.book_append_sheet(
    ...
    XLSX.utils.json_to_sheet(
      installments.map((row) => ({
        ...
        "Paid": row.settledAmount,
        "Receipted here": row.appliedAmount,
