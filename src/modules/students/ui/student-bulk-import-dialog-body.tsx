...
import type { ImportBatchDialogSummary, ImportMode } from "@/modules/imports/domain/types";
...
/**
 * The dialog itself: session picker, file upload, validation preview, commit.
 *
 * Split out of the trigger so it is `next/dynamic`-loadable. It was 21 KB of
 * source in the initial Students bundle -- an upload flow that most visits to
 * the roll never open, paid for on every page load, phones included.
 *
 * Mounted only while open, so opening it is also what resets the form. The
 * trigger used to do that by hand in openDialog(); a fresh mount does it for
 * free and cannot drift out of sync with the state list.
 */
export function StudentBulkImportDialogBody({
  mode,
  sessionOptions,
  defaultSessionLabel,
  onClose,
  ...
  const [sessionLabel, setSessionLabel] = useState(
    mode === "update" ? "__all__" : defaultSessionLabel,
  );
