 ...
 *
 * One endpoint, one job: fire an approved WhatsApp template at one number by
 * naming a Live API Campaign. The template body and its variable slots live in
 * AiSensy and Meta, not here — all this sends is the campaign name and the
 * values that fill the slots.
 *
 * The Basic plan includes this endpoint. The richer "Project API" (contact
 * CRUD, delivery webhooks) is Pro-only, which is why nothing here tries to read
 * delivery status back: on Basic, `submitted_message_id` is an acceptance
 * receipt, not proof the parent's phone lit up. Check the AiSensy dashboard for
 * that.
 */
...
export async function sendAisensyCampaignMessage(
  ...
  try {
    ...
    // A network failure is not a delivery failure — we genuinely do not know
    // whether AiSensy accepted it. The caller records this as failed and the
    // day stays claimed, which is the safe direction: better one parent missed
    // than one parent messaged twice.
    return {
      ok: false,
      status: 0,
 ...
 *
 * Returns null for anything that is not recognisably an Indian mobile. That
 * null is load-bearing: every send costs money and every message to a wrong
 * number is a stranger receiving a child's name and fee balance.
 */
