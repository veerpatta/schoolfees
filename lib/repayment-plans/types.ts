...
export type RepaymentPlanScope = "old_balance_only" | "old_and_current";
...
export type RepaymentPlanCollectionContext = {
  ...
  /**
   * True for `old_balance_only`. Current-year dues stay unpaid until the old
   * balance clears, and the cashier is told so before taking the money.
   */
  currentYearRemainsUnpaid: boolean;
};
...
export function isRepaymentPlanScope(value: unknown): value is RepaymentPlanScope {
  return value === "old_balance_only" || value === "old_and_current";
}
