-- Exclude discount-mode receipts from "collection" KPIs.
--
-- Discount close-outs (payment_mode = 'discount') are write-offs, not cash.
-- They reduce dues/pending (handled in 20260526100000) but must not appear
-- in collected/today/this-month/trend/heatmap/mode-breakdown/recent-payments
-- numbers. Patch get_dashboard_summary to filter them out at the source.
--
-- This is a CREATE OR REPLACE of the body originally defined in
-- 20260524182924_dedupe_dashboard_installment_labels_v2.sql with the
-- following surgical changes: every receipts-table query gains
