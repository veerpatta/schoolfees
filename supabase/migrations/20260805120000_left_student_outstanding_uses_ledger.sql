-- Withdrawn students reported their full annual fee as outstanding.
--
-- v_workbook_student_financials aggregates each student's money from
-- v_workbook_installment_balances, which excludes cancelled installments. When a
-- student is withdrawn the app cancels every clean unpaid installment
-- (lib/fees/generator.ts:532-583), so such a student can end up with NO rows in
-- the balances view at all. The LEFT JOIN then yields NULL and the COALESCE
-- fallback handed back the policy-derived annual fee -- the student appeared to
-- owe a full year precisely because their dues had been cancelled. Observed
-- live: 28 withdrawn students reporting Rs 5,43,750 against Rs 22,750 of real
-- remaining balance.
--
-- The fallback exists so students whose dues have not been generated yet still
-- show an expected fee, so it is kept -- but it now applies only when the
-- student has no installment rows at all. Once a ledger exists the ledger is the
-- truth, and an absent summary means zero rather than "charge the full year".
--
-- Deliberately status-agnostic: an ACTIVE student whose installments were all
-- cancelled was affected in exactly the same way.
--
-- Dashboard and Defaulters were never wrong -- they filter record_status =
-- 'active' -- so no active-student total moves. Exports, the AI context bundle
-- and per-student views were affected.
--
