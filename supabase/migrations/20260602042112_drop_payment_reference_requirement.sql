-- Drop the mandatory reference-number requirement from payment posting.
--
-- WHY:
--   The Payment Desk UI no longer collects a reference number (the field was
--   removed; the form posts an empty reference for every mode). The server
--   preflight already tolerates a null reference. The only remaining gate was
--   inside post_student_payment_with_adjustments, which rejected UPI / bank
--   transfer / cheque posts with:
--     P0001 "Reference number is required for UPI, bank transfer, and cheque payments."
--   That blocked all non-cash payment posting. Reference number is now optional
--   for every payment mode; the column stays nullable and is still persisted
--   when supplied.
--
-- SAFETY:
--   * Body is byte-identical to migration 20260528151933 EXCEPT the reference
--     guard block is removed. security invoker preserved. Audit fixes
--     (1.3 idempotency re-check, 1.29 null per-installment notes, 1.30 cached
--     snapshot temp table, 1.5 advisory lock) all preserved.
...
begin
  ...
  -- Reference number is optional for all payment modes (UI no longer collects it).
