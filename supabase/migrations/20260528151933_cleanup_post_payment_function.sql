-- Audit 1.29 + 1.30 — Cleanup pass on post_student_payment_with_adjustments.
--
-- WHY:
--   1.29 — The current function stamps p_remarks on receipts.notes AND on
--          every payments.notes row AND on every receipt_adjustments.notes
--          row in the per-installment loop. Long remarks balloon the
--          exports and the duplication serves no audit purpose — the
--          receipt-level note is canonical; per-installment notes should
--          only carry per-installment context (which the Payment Desk does
--          not collect today). Per-installment columns are nulled.
--
--   1.30 — `private.workbook_installment_snapshot(p_student_id, p_payment_
--          date, true)` is called twice (once for v_total_pending, once
--          inside the allocation loop). Postgres `stable` is a planner hint,
--          not a guarantee. Cheap defensive fix: cache once into a temp
--          table and reuse.
--
-- This migration ALSO preserves audit 1.3's idempotency re-check inside the
-- unique_violation handler so it stays correct after the cleanup.
--
-- SAFETY:
--   * Function signature is unchanged.
--   * Allocation-snapshot columns (audit 1.3) preserved.
--   * Idempotency re-check (audit 1.3) preserved.
--   * Advisory lock (audit 1.5 sibling salt) preserved.
...
begin
  ...
  -- Audit 1.30 — cache the snapshot once into a temp table so the two
  -- downstream consumers (the total-pending sum and the per-installment
  -- allocation loop) read the same row set instead of re-running the
  -- expensive `private.workbook_installment_snapshot` function.
  ...
  for _attempt in 1..12 loop
    ...
    exception when unique_violation then
      -- Audit 1.3 — preserved here. Before retrying with a new receipt-
      -- number, check whether another concurrent call (with the same
      -- client_request_id) already committed a receipt for this student.
      if p_client_request_id is not null then
        select r.id, r.receipt_number, r.total_amount
        into v_candidate_receipt_id, v_existing_receipt_number, v_existing_total_amount
        from public.receipts as r
  ...
  loop
    ...
    if v_discount_allocation > 0 then
      -- Audit 1.29 — per-installment notes are nulled; receipts.notes is the
      -- canonical audit string. The 'reason' column captures the discount/
      -- waiver context already.
    ...
    if v_payment_allocation > 0 then
      ...
      -- Audit 1.29 — payments.notes is null. receipt-level notes are the
      -- canonical audit string; duplicating per-installment ballooned exports.
