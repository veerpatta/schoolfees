...
-- 2. Create public.v_workbook_student_financials as a Materialized View
...
-- 3. Create public.v_student_financial_state as a Materialized View
...
-- 5. Create the trigger refresh function
...
as $$
begin
  -- Run non-concurrent refresh inside trigger firing transactions
  perform public.refresh_financial_materialized_views(false);
  return null;
end;
$$;

-- Triggers for automatic materialization refresh on source mutations
...
-- Let PGRST know that view definitions (which are now materialized views) have changed
