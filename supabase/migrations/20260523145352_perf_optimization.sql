-- Migration to add performance indexes and a consolidated dashboard summary RPC.

-- 1. Create performance indexes
CREATE INDEX IF NOT EXISTS idx_payments_created_at 
  ON public.payments (created_at DESC);

CREATE INDEX IF NOT EXISTS idx_test_payments_created_at 
  ON test.payments (created_at DESC);

CREATE INDEX IF NOT EXISTS idx_students_class_id 
  ON public.students (class_id);

CREATE INDEX IF NOT EXISTS idx_test_students_class_id 
  ON test.students (class_id);

CREATE INDEX IF NOT EXISTS idx_installments_student_class 
  ON public.installments (student_id, class_id);

CREATE INDEX IF NOT EXISTS idx_test_installments_student_class 
  ON test.installments (student_id, class_id);

-- 2. Create the consolidated get_dashboard_summary RPC
...
begin
  ...
  from (
    ...
    group by installment_no, installment_label, due_date
    order by installment_no
  ) t;

  -- I. Class Installment Matrix
  select coalesce(json_agg(t), '[]'::json) into v_class_installment_matrix
  from (
    with distinct_installments as (
      select distinct installment_no, installment_label
      from public.v_workbook_installment_balances
      where session_label = p_session_label
    ),
...
-- Grant permissions to access the RPC
