-- Exclude discount-mode receipts from "paid" totals in the workbook view layer.
--
-- Background: the Close-balance-as-discount feature in
-- app/protected/students/close-due-actions.ts creates a receipt with
-- payment_mode='discount' and allocates it across pending installments via
-- the payments table so the workbook view drives pending to zero. The view
-- previously summed every payment regardless of mode, which inflated
-- total_paid everywhere (dashboard KPIs, Day Close, exports, reports) and
-- caused the late_fee gating to incorrectly trigger off the close-out
-- receipt (since had_payment_after_due was true for the discount payment).
--
-- This migration splits the per-installment payment sum into:
--   paid_amount             — receipts where payment_mode != 'discount' (cash)
--   discount_closeout_amount — receipts where payment_mode  = 'discount'
-- and keeps pending_amount computed from both, so close-outs still bring
-- the balance to zero.
--
-- Notes:
--   * paid_by_due_amount and had_payment_after_due exclude discount-mode
--     receipts, so a discount close-out does not retroactively trigger a
--     late fee on an installment that had no real cash payment.
--   * last_payment_date still considers all receipts (close-outs are real
--     finance events).
--   * applied_amount keeps its existing meaning (cash + adjustments capped
--     at 0); pending subtracts discount_closeout_amount as a separate term
--     so consumers reading applied_amount keep seeing cash-only credit.
--   * The snapshot function signature is unchanged (no new return column)
--     to avoid touching preview_workbook_payment_allocation and its many
--     callers; the view exposes discount_closeout_amount as a new column.

-- Drop dependent materialized views in correct order.
do $$
begin
...
-- 1. Recreate private.workbook_installment_snapshot with split payment sum.
...
-- 2. Recreate v_workbook_installment_balances as materialized view, splitting
--    the payment sum and adding discount_closeout_amount as a new column.
...
-- 3. Recreate v_workbook_student_financials, adding total_discount_closeouts.
...
-- 4. Recreate v_student_financial_state (unchanged shape; depends on financials).
...
-- Unique indexes for concurrent refreshes (mirrors 20260523130000).
...
create unique index if not exists v_student_financial_state_idx
  on public.v_student_financial_state (student_id);

-- Session-filter indexes (mirrors 20260523213000_tier3_finance_performance).
