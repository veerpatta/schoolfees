-- Let the correction harness waive a late fee.
--
-- `waive_late_fee` guards on `has_permission('payments:waive_late_fee')`, which
-- needs `auth.uid()` and is therefore always false under the service role. That
-- was fine while every waiver came from the Payment Desk. It stopped being fine
-- the moment a fee-correction register arrived with "late fee is not included"
-- written across it: the corrections could be applied headlessly and the waivers
-- could not, which is the worst of both — the ledger moves and the late fee it
-- implies does not.
--
-- Two guards gain a service-role arm, and nothing else changes. This is the same
-- shape `reverse_receipt_admin` and `get_dashboard_repayment_summary` already
-- use, and it is deliberately an ARM rather than a bypass: a browser session
-- still needs the permission, and an EMI plan still needs `fees:repayment_plan`.
--
-- One consequence worth knowing. `waived_by` is `auth.uid()`, which is null for
-- a headless caller, so a waiver written this way records no approver. The
-- reason text is the trail instead — the harness passes one naming the register
-- and the run — and `scripts/bulk-apply.mjs` writes its own `audit_logs` row on
-- top. A waiver with no reason is refused by the length check below, as before.
...
begin
  -- >>> CHANGED: service-role arm for the headless correction harness. <<<
  ...
  if exists (
     ...
     -- >>> CHANGED: the harness is trusted here for the same reason an admin is. <<<
