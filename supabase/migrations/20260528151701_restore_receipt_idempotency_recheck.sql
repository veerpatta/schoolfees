-- Audit 1.3 — Restore the idempotency re-check inside the receipt-number
-- retry loop of post_student_payment_with_adjustments.
--
-- WHY:
--   The earlier 20260425143000 migration of post_student_payment had a guard
--   inside the `exception when unique_violation` branch: it re-queried by
--   (student_id, client_request_id) and returned the existing receipt cleanly
--   if one was found. The 20260527033430 rewrite (which added the allocation
--   snapshot columns) lost that guard — the catch block now just `continue`s
--   without looking up the existing receipt. With 12 attempts the second call
--   exhausts the loop and raises "Unable to generate a unique receipt number"
--   even though the first call succeeded for the same client_request_id.
--
-- SCENARIO:
--   1. The client posts a payment; the network drops the response.
--   2. The client retries with the same client_request_id.
--   3. Both calls pass the upfront `if p_client_request_id is not null` check
--      (because the first insert hasn't committed yet when the second snapshots).
--   4. One wins the receipt-number race. The other catches unique_violation,
--      bumps v_daily_sequence, and tries 12 new numbers. All 12 collide.
--   5. The user sees a confusing "Unable to generate a unique receipt number"
--      and is likely to attempt the post a third time.
--
-- FIX:
--   Inside `exception when unique_violation`, look up the existing receipt by
--   (student_id, client_request_id). If found, return it cleanly. Mirror the
--   pattern from 20260425143000_payment_desk_idempotency_and_locking.sql:186-202.
--
-- SAFETY:
--   * Function signature is unchanged.
--   * Columns added by 20260527033430 (discount_applied_at_posting,
--     waiver_applied_at_posting, pending_before_posting, pending_after_posting)
--     are preserved.
--   * All other validation, locking, allocation, and audit logic is unchanged.
--   * Apply to TEST Supabase branch first; never run against the production
--     project_ref without explicit user approval per
...
begin
  ...
  for _attempt in 1..12 loop
    ...
    exception when unique_violation then
      -- Audit 1.3 — before retrying with a new receipt-number, check whether
      -- another concurrent call (with the same client_request_id) already
      -- committed a receipt for this student. If so, return it cleanly so the
      -- retry sees a successful posting instead of "Unable to generate a
      -- unique receipt number" on the 12th attempt.
      if p_client_request_id is not null then
        select r.id, r.receipt_number, r.total_amount
        into v_candidate_receipt_id, v_existing_receipt_number, v_existing_total_amount
        from public.receipts as r
