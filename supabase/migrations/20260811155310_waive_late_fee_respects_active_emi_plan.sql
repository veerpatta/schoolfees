-- Close a bypass found in smoke testing.
--
-- post_student_payment_with_adjustments refuses concessions while an EMI plan
-- is active, but the Payment Desk never sends the waiver through that RPC: it
-- calls waive_late_fee FIRST and then posts with p_quick_late_fee_waiver_amount
-- = 0. So the guard never saw the desk's waiver, and only the hidden UI control
-- stood between a cashier and forgiving a fee on a family with an agreed plan.
--
-- Refuses for anyone who cannot reschedule the plan, which is the spec's
-- "requires admin rescheduling". Admins keep the escape hatch: an
-- old-balance-only plan leaves current-year installments uncovered, and those
-- can still legitimately need a waiver.
--
-- Returns ok=false rather than raising, matching how this function already
-- reports refusals, so the desk shows the reason instead of a stack trace.
-- Everything except the new guard is the body exactly as it stood before.
