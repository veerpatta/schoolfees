...
declare
  ...
  import_name constant text := 'latest-excel-import-2026-05-14-201835-direct-2026-27-safe-matched';
...
begin
  ...
  if p_kind = 'students' then
    ...
    select import_name, row->>'sourceKey', row
    ...
    select import_name, row->>'source', (row->>'sourceRowNumber')::integer, row->>'status', row
    from jsonb_array_elements(p_rows) as row
  ...
  return jsonb_build_object(
    ...
    'students_total', (select count(*) from private.vpps_direct_import_stage_students where import_name = import_name),
    'dues_total', (select count(*) from private.vpps_direct_import_stage_dues where import_name = import_name),
    'skipped_total', (select count(*) from private.vpps_direct_import_stage_skipped where import_name = import_name)
  );
end;
$$;
...
comment on function public.vpps_stage_direct_import_20260514(text, text, jsonb) is 'Temporary guarded staging function for the approved 2026-27 latest Excel import. Drop after use.';
