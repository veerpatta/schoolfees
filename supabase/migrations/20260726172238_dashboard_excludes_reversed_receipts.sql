-- Stop counting fully reversed receipts as money.
--
-- Every receipt-sourced aggregate in this function filtered only
-- payment_mode <> 'discount'. Nothing excluded a reversed receipt, and a
-- reversal never touches receipts.total_amount -- it inserts a compensating
-- payment_adjustments row. So a receipt that had been reversed still counted
-- as collected.
--
-- Live symptom on 2026-27: the two receipts posted and reversed on 2026-07-26
-- kept the dashboard reporting Rs 11,000 collected today. Meanwhile
-- totalCollected reads from v_workbook_student_financials, whose
-- applied_amount DOES net adjustments -- so the same page showed Rs 11,000
-- taken today next to a year-to-date total that had already excluded it, and
-- collectionRate silently followed the netted side.
--
-- Six aggregates are corrected: todaysCollection + receiptsToday,
-- thisMonthCollection, todayPaymentModeBreakdown, recentPayments,
-- collectionTrend and collectionHeatmap.
--
-- Verified against live before applying:
--   today  Rs 11,000   -> Rs 0   (2 receipts -> 0)
--   month  Rs 2,49,625 -> Rs 2,00,200
--
-- PARTIAL reversals still count. A receipt reversed for less than its total
-- represents money that really did arrive, and this matches isReceiptReversed()
-- in lib/receipts/reversals.ts, which is what every badge in the app already
-- uses to decide "voided".
--
-- SECURITY: v_receipt_reversal_totals is security_invoker while this function
-- is SECURITY DEFINER, so inside here the view evaluates as the definer and
-- sees every receipt. That is exactly what the surrounding queries already do
-- -- NO PRIVILEGE IS WIDENED. The predicate only removes rows from a total; it
-- can never expose a row to a caller who could not already count it.

CREATE OR REPLACE FUNCTION public.get_dashboard_summary(p_session_label text, p_today text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
-- MUST be restated. CREATE OR REPLACE FUNCTION replaces the whole definition
-- including its config settings, so omitting this would silently undo the
-- search_path hardening applied by 20260523164957 to every SECURITY DEFINER
-- function. The body also calls private.normalize_workbook_class_label, which
-- needs `private` on the path.
...
begin
  ...
  select coalesce(sum(total_due), 0)::integer, coalesce(sum(total_paid), 0)::integer, coalesce(sum(outstanding_amount), 0)::integer
  ...
  select coalesce(json_agg(t), '[]'::json) into v_class_summary from (
    ...
    class_financials as (
      select class_id, count(*)::integer as students_with_generated_dues,
        sum(total_due)::integer as expected_amount, sum(total_paid)::integer as collected_amount,
  ...
  select coalesce(json_agg(t), '[]'::json) into v_installment_summary from (
    select installment_no as "installmentNo", max(installment_label) as "installmentLabel", due_date::text as "dueDate",
      count(distinct student_id)::integer as "studentCount", sum(total_charge)::integer as "expectedAmount",
