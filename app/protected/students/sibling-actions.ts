...
export async function linkSuspectedSiblingsAction(
  ...
  const group = (groupRows ?? [])[0] as
    ...
    | undefined;
  const suspectedIds = uniqueIds(group?.student_ids ?? []);

  if (suspectedIds.length < 2) {
  ...
  const { data: existingRaw, error: existingError } = await supabase
    .from("student_family_members")
    .select("family_group_id, student_id, academic_session_label, sibling_order")
    .in("student_id", suspectedIds);

  if (existingError) {
    return {
      status: "error",
      message: `Unable to check existing family groups: ${existingError.message}`,
      familyGroupId: null,
    };
  }
  ...
  try {
    const resolved = await resolveFamilyGroupId(
      supabase,
      existing.map((row) => row.family_group_id),
    );
    mergedGroupCount = resolved.mergedGroupCount;
    familyGroupId =
      resolved.familyGroupId ??
      (await createFamilyGroup(supabase, {
        sessionLabel,
        anchorStudentId: studentId,
        guardianName,
        guardianPhone,
  ...
  try {
    memberIds = await backfillFamilyMembershipForSession(supabase, {
      familyGroupId,
      sessionLabel,
      studentIds: suspectedIds,
      notes: "Linked from suspected siblings on the student profile.",
    });
  } catch (error) {
    return {
      status: "error",
      message: `Unable to link siblings: ${(error as Error).message}`,
      familyGroupId,
    };
  }
  ...
  return {
    status: "success",
    message: `Linked ${memberIds.length} students as a family.${mergeNote}${describeThirdChildOutcome(thirdChildResult, nameById)}`,
    familyGroupId,
  };
}
